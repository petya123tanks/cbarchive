...
library:CreateWatermark("SaladWare | Version 4.0 | L0S3RC0UNT3R#2452 ")
library.theme.accentcolor = Color3.fromRGB(1, 50, 0)
library.theme.accentcolor2 = Color3.fromRGB(1, 100, 0)
library.theme.topheight = 60
library.theme.TextColor = Color3.fromRGB(1, 0, 0)
...
function library:New(name) 
	...
	do 
		...
		TextLabel.TextColor3 = COL3RGB(2, 1, 1) 
...
local gui = library:New("SaladWare") 
...
local credits = gui:Tab("Made By L0S3RC0UNT3R#2452")
...
luascripts:Element("Button", "Saladwere", nil, function()
...
local function AddLegit(Tab) 
	...
	Tab:Element("Slider", "minimum dmg(Bruh)", {min = 0, max = 100, default = 15}) 
end 
...
antiaim:Element("Dropdown", "pitch", {options = {"up", "down", "imposter", "random", "salad",}}) 
...
others:Element("Toggle", "remove head (IshMocha moment)") 
...
Text.TextColor3 = Color3.fromRGB(3020, 42060, 2482)
...
movement:Element("Dropdown", "type", {options = {"gyro", "cframe", "Saladware"}}) 
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do 
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "Winning" and "SaladWere winning$$$$$" or "Join saladwere https://discord.gg/xstjreNx", false, "Innocent", false, true) 
...
SampleLabel.BackgroundColor3 = Color3.fromRGB(55, 22, 500)
...
SampleLabel.TextColor3 = Color3.fromRGB(55, 100, 22)
...
TeamDamage = false
griefsector:Element("ToggleKeybind","Show teams damage",{},function(tbl)
...
RunService.RenderStepped:Connect(function(step) 
	...
	if PlayerIsAlive then 
		...
		if values.rage.exploits["kill all(Gay)"].Toggle and values.rage.exploits["kill all(Gay)"].Active and LocalPlayer.Character:FindFirstChild("UpperTorso") and LocalPlayer.Character:FindFirstChild("Gun") then
			for _,Player in pairs(Players:GetPlayers()) do
		...
		if values.rage.angles.enabled.Toggle and not DisableAA then 
			...
			local Pitch = values.rage.angles["pitch"].Dropdown == "up" and 1 or values.rage.angles["pitch"].Dropdown == "down" and -1 or values.rage.angles["pitch"].Dropdown == "salad" and -6 or values.rage.angles["pitch"].Dropdown == "random" and RANDOM(-10, 10)/10 or 2.5
			if values.rage.angles["extend pitch"].Toggle and (values.rage.angles["pitch"].Dropdown == "up" or values.rage.angles["pitch"].Dropdown == "down" or values.rage.angles["pitch"].Dropdown == "salad" or values.rage.angles["pitch"].Dropdown == "180") then 
				Pitch = (Pitch*2)/1.6 
			end 
