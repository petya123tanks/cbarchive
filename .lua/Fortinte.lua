getgenv().text = "Stormy | Ver: 1.0.4 | Status: Private"
...
local cfglocation = "Better_Stormycfg/"      
makefolder("Better_Stormycfg")      
...
local MainUIColor = COL3RGB(169, 169, 169)    
		...
		function Tab:Sector(text, side)      
			...
			function Sector:Element(type, text, data, callback)      
				...
				if type == "ScrollDrop" then      
					...
					Dropdown.BackgroundColor3 = COL3RGB(255, 255, 255)      
					Dropdown.BackgroundTransparency = 1.000      
					Dropdown.Position = UDIM2(0, 0, 0, 1)      
					Dropdown.Size = UDIM2(1, 0, 0, 39)      
					...
					for i,v in ipairs(joe) do      
						do      
							...
							Frame.CanvasSize = UDIM2(0, 0, 0, 1)      
...
local function UpdateWeapon(obj)      
	...
	obj.Material = selected == "Smooth" and "SmoothPlastic" or selected == "Neon" and "Neon" or selected == "ForceField" and "ForceField" or "Glass"      
...
makefolder("Better_Stormylua")      

local allluas = {}      

for _,lua in pairs(listfiles("Better_Stormylua")) do      
	local luaname = GSUB(lua, "Better_Stormylua\\", "")      
	INSERT(allluas, luaname)      
end      

RunService.RenderStepped:Wait()      

local gui = library:New("Stormy 1.0.4")      
...
luascripts:Element("Button", "load", {}, function()      
	loadstring(readfile("bloxsenselua\\"..values.luas["lua scripts"].lua.Scroll))()      
end)      
...
aimbot:Element("Jumbobox", "resolver", {options = {"pitch", "roll", "Bhop","animation"}})      
		...
		pcall(function()
			...
			for i,v in pairs(debug.getupvalues(Client.setcharacter)) do
				if type(v) == "userdata" and v.ClassName == "AnimationTrack" and v.Name == "Idle" then
					CrouchAnim = v																																																																																																																																																																																																																																																																																																																																																						-- this_was_pasted_from_coolhack
				end
			end
...
effects:Element("Dropdown", "weapon material", {options = {"Smooth", "Neon", "ForceField", "Glass"}}, function(tbl)      
	if WeaponObj == nil then return end      
...
world:Element("Dropdown", "hitsound", {options = {"none", "bloxsense", "neverlose", "rust", "bag", "baimware","cod"}})      
...
chat:Element("Toggle", "chat spam", nil, function(tbl)      
	if tbl.Toggle then      
		while values.misc.chat["chat spam"].Toggle do      
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "hi" or "bloxsense > all!", false, "Innocent", false, true)      
...
grenades:Element("Button", "crash server", {}, function()      
	while true do
        pcall(function()
            game:GetService("RunService").RenderStepped:Wait()
            for i = 1,100,1 do
                local ohInstance1 = LocalPlayer.Character.Gun.Mag																																																																									-- Server Crasher exploit by property (bloxsense). mis hermanos solo usan bloxsense
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)      
	...
	if values.misc.chat["kill say"].Toggle then      
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "nn fue destruido por bloxsense. propiedad de p100 cheat (bloxsense)", false, "Innocent", false, true)      
	end      
end)      
