...
function library:New(name)
	...
	local baran = Instance.new("ScreenGui")
	...
	baran.Name = "baran"
	baran.ResetOnSpawn = false
	baran.ZIndexBehavior = "Global"
	baran.DisplayOrder = 420133769

	local UIScale = Instance.new("UIScale")
	UIScale.Parent = baran
    ...
    game:GetService("RunService").RenderStepped:connect(function()
        cursor.Visible = baran.Enabled
        cursor.Position = UDim2.new(0,Mouse.X-3,0,Mouse.Y+1)
    end)

	Menu.Name = "Menu"
	Menu.Parent = baran
	Menu.BackgroundColor3 = Color3.fromRGB(8, 35, 5)
	Menu.Position = UDim2.new(0.5, -300, 0.5, -300)
	Menu.Size = UDim2.new(0, 600, 0, 700)
	Menu.Image = "rbxassetid://14854782566"
    Menu.ImageTransparency = 0.89
	library.uiopen = true

	game:GetService("UserInputService").InputBegan:Connect(function(key)
		if key.KeyCode == Enum.KeyCode.Insert then
			baran.Enabled = not baran.Enabled
			library.uiopen = baran.Enabled
		end
	end)
...
for _,lua in pairs(listfiles("baranlua")) do
	local luaname = string.gsub(lua, "baranlua\\", "")
	table.insert(allluas, luaname)
end

RunService.RenderStepped:Wait()

local gui = library:New("BARANWARE")
...
luascripts:Element("Button", "load", {}, function()
	loadstring(readfile("baranlua\\"..values.luas["lua scripts"].lua.Scroll))()
end)

local Commands = luas:Sector("Commands", "Right")
Commands:Element("Button", "Rejoin", {}, function()
    local ts = game:GetService("TeleportService")
    local p = game:GetService("Players").LocalPlayer
    ts:Teleport(game.PlaceId, p)
end)

Commands:Element("Button", "Respawn", nil, function()
...
aimbot:Element("Dropdown", "automatic fire", {options = {"off", "standard", "hitpart", "instant-shoot"}})     
...
antiaim:Element("Dropdown", "pitch", {options = {"none", "up", "down", "zero", "random"}})      
...
antiaim:Element('Dropdown', 'onshot mode', {options = {"none", "up", "down", "zero", "random"}})
local others = legit:Sector("others", "Right")
...
client:Element("Jumbobox", "gun modifiers", {options = {"spread", "reload", "equip", "ammo", "automatic", "penetration", "firerate"}})
...
movement:Element("Slider", "speed", {min = 15, max = 100, default = 15})
...
chat:Element("Toggle", "chat spam", nil, function(tbl)
	if tbl.Toggle then
		while values.misc.chat["chat spam"].Toggle do
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "1" or "BARANWARE: БЭЭЭЭ БЭЭЭЭ БЭЭЭЭ", false, "Innocent", false, true)
			wait(values.misc.chat["speed (ms)"].Slider/1000)
		end
	end
end)
chat:Element("Dropdown", "type", {options = {"standard", "BARANWARE"}})
...
mt.__namecall = function(self, ...)
	...
	if method == "LoadAnimation" and self.Name == "Humanoid" then
		if values.legit.others["leg movement"].Dropdown == "slide" then
            if string.find(args[1].Name, "Walk") or string.find(args[1].Name, "Run") then
                args[1] = FakeAnim
            end
        end
        if values.legit.others["no animations"].Toggle then
            args[1] = FakeAnim
        end
	end
    if method == "FireServer" and self.Name == "Standard" then
...
CreateHitElement(" DONE! ",Color3.new(1,1,1),3, 0, 245, 0, 22)