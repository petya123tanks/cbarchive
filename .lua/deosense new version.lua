...
function library:New(name) 
	...
	Deo.Name = "electric boogalo" 
	Deo.ResetOnSpawn = false 
	Deo.ZIndexBehavior = "Global" 
	Deo.DisplayOrder = 420133769 
	...
	game:GetService("UserInputService").InputBegan:Connect(function(key) 
		if key.KeyCode == Enum.KeyCode.Insert then 
			Deo.Enabled = not Deo.Enabled 
			library.uiopen = Deo.Enabled 
		end 
	end) 
	...
	do 
		...
		TextLabel.BorderColor3 = COL3RGB(255, 225, 0) 
	...
	TextLabel.Font = Enum.Font.SourceSansSemibold 
	TextLabel.Text = name 
	TextLabel.TextColor3 = COL3RGB(255, 225, 0) 
	...
	function menu:Tab(text) 
		...
		TextButton.TextColor3 = COL3RGB(255, 225, 0) 
			...
			function MSector:Tab(text) 
				...
				function tab:Element(type, text, data, callback) 
					...
					if type == "Jumbobox" then 
						...
						Drop.ZIndex = 5 
						Drop.ScrollBarImageColor3 = COL3RGB(255, 225, 0) 
							...
							do 
								...
								Button.MouseButton1Down:Connect(function() 
									if TBLFIND(Element.value.Jumbobox, v) then 
										...
										library:Tween(TextLabel, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(255, 225, 0)}) 
									end 
									updatetext() 
							...
							for i,v in pairs(Drop:GetChildren()) do 
								if v.Name ~= "UIListLayout" then 
									if TBLFIND(val.Jumbobox, v.Name) then 
										v.TextLabel.TextColor3 = COL3RGB(255, 225, 0) 
									else 
					...
					elseif type == "ToggleKeybind" then 
						...
						Color.Name = "Color" 
						Color.Parent = Button 
						Color.BackgroundColor3 = COL3RGB(255, 225, 0) 
						...
						local function update() 
							if Element.value.Toggle then 
								tween = library:Tween(Color, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = COL3RGB(255, 225, 0)}) 
								library:Tween(TextLabel, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(255, 255, 255)}) 
							else 
								keybindremove(text) 
						...
						do 
							...
							Always.Text = "Always" 
							Always.TextColor3 = COL3RGB(255, 225, 0) 
							...
							for _,button in pairs(Frame:GetChildren()) do 
								if button:IsA("TextButton") then 
									button.MouseButton1Down:Connect(function() 
										...
										button.TextColor3 = COL3RGB(255, 225, 0) 
										values[tabname][sectorname][tabtext][text] = Element.value 
										callback(Element.value) 
									end) 
									button.MouseEnter:Connect(function() 
					...
					elseif type == "Slider" then 
						...
						UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, COL3RGB(153, 22, 65)), ColorSequenceKeypoint.new(1, COL3RGB(255, 225, 0))} 
		...
		function Tab:Sector(text, side) 
			...
			function Sector:Element(type, text, data, callback) 
				...
				if type == "ScrollDrop" then 
					...
					for i,v in ipairs(joe) do 
						do 
							...
							Frame.ScrollBarImageColor3 = COL3RGB(255, 225, 0)      
								...
								for _,bad in pairs(v.Frame:GetChildren()) do      
									if bad:IsA("TextButton") then      
										...
										if bad.Name == Element.value.Scroll[v.Name] then      
											bad.TextLabel.TextColor3 = COL3RGB(255, 225, 0)      
										end      
									end      
								end      
							end      
				...
				elseif type == "Slider" then 
					...
					UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, COL3RGB(255, 225, 0)), ColorSequenceKeypoint.new(1, COL3RGB(255, 225, 0))} 
					UIGradient.Rotation = 90 
					UIGradient.Parent = Frame 
...
for _,lua in pairs(listfiles("senselua")) do 
	local luaname = GSUB(lua, "senselua\\", "") 
	INSERT(allluas, luaname) 
end 

RunService.RenderStepped:Wait() 

local gui = library:New("deosense$$$") 
...
luascripts:Element("Button", "load", {}, function() 
	loadstring(readfile("senselua\\"..values.luas["lua scripts"].lua.Scroll))() 
end) 
...
fakelag:Element("Slider", "lag tick", {min = 1, max = 300, default = 8}) 
fakelag:Element("Dropdown", "amount", {options = {"static", "dynamic"}}) 
fakelag:Element("Slider", "limit", {min = 1, max = 300, default = 8}) 
...
exploits:Element("Slider", "quick peek vertical pos", {min = -100, max = 1000, default = 200})  
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do 
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "get good,get deosense" or "deosense$$$", false, "Innocent", false, true) 
			wait(values.misc.chat["speed (ms)"].Slider/1000) 
			 
		end 
	end 
end) 
chat:Element("Dropdown", "type", {options = {"standard", "deosense$$$"}}) 
...
Camera.ChildAdded:Connect(function(obj) 
	if TBLFIND(values.misc.client["gun modifiers"].Jumbobox, "ammo") then 
		Client.ammocount = 99969 
		Client.primarystored = 99969
		Client.ammocount2 = 99969 
		Client.secondarystored = 99969 
	end 
	RunService.RenderStepped:Wait() 
...
LocalPlayer.Cash:GetPropertyChangedSignal("Value"):Connect(function(cash) 
	if values.misc.client["infinite cash"].Toggle and cash ~= 99999 then 
