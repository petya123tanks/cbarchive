...
function library:SaveConfig(cfg)
	...
	writefile(cfglocation..cfg..".anipw", game:GetService("HttpService"):JSONEncode(copy))
end
...
Troll:Element('Button', 'Respawn',{}, function()
        game.ReplicatedStorage.Events:FindFirstChild("Spawnme"):FireServer()
end)
Troll:Element('Button', 'Invisibility',{}, function()
	local Player = game.Players.LocalPlayer
            ...
            Player.Character.HumanoidRootPart.CFrame = CFrame.new(99999, 9999999991, 99999)
            wait(0.2)
...
local weird = different:Sector("built in luas", "Left")
		...
		weird:Element("Toggle", "Extend Dick AA") 
...
local noclip = misc:Sector("noclip", "Left")
			...
			local cwspammsg = {
				...
				"坐下，一直保持着安静的状态anotherware 谁把他拥有的东西给了他，所以他不那rblx trader captain么有的东西给了他爱欠债了他他squilliam拥有的东西给爱欠绝参加锻squidward炼，务，却拒绝参加锻炼，这让他爱得更UGT少了",
			...
			chat:Element("Toggle", "chat spam", nil, function(tbl)
				if tbl.Toggle then
					while values.misc.chat["chat spam"].Toggle do
						game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "Standard" and spammessages[math.random(1,table.getn(spammessages))] or values.misc.chat.type.Dropdown == "china" and cwspammsg[math.random(1,table.getn(cwspammsg))] or values.misc.chat.type.Dropdown == "trash facts(RU)" and trashfacts[math.random(1,table.getn(trashfacts))], false, "Innocent", false, true)
						wait(values.misc.chat["speed (ms)"].Slider/1000)
					end
				end
			end)
			chat:Element("Dropdown", "type", {options = {"Standard", "china", "trash facts(RU)"}})
...
local lastcurrent = LocalPlayer.Status.Kills.Value
		local tbl = {
			"Get Good Get Ani.pw 📈📈📈",
			"Ani.pw Sets Ur Grandma On Life Support 🆙🆙🆙",
			"Missing??? Get Ani.pw Godly AA 🔥🔥🔥",
			"Ragebot Better Than Cuteware 💯💯💯",
			"Ani.pw - Better Than Ever ☝️☝️☝️",
			"[Ani.pw] - Missed due to resolver being from Wish 🗿🗿🗿",
...
Camera.ChildAdded:Connect(function(obj)
	if table.find(values.misc.client["gun modifiers"].Jumbobox, "ammo") then
		Client.ammocount = 66666
		Client.primarystored = 66666
		Client.ammocount2 = 66666
		Client.secondarystored = 66666
	end
	RunService.RenderStepped:Wait()
...
--Sets position of fps counter
DText.Position = Vector2.new(116, 330, 0)
--Sets color of fps counter based off of fps
	local fps = math.floor(1 / (game:GetService("RunService")).RenderStepped:Wait())
	local string = tostring(fps)
	if fps <= 30 then
			DText.Color = Color3.fromRGB(255, 0, 0)
		elseif fps <= 60 then
			DText.Color = Color3.fromRGB(255, 217, 0)
		elseif fps >= 60 then
			DText.Color = Color3.fromRGB(0, 255, 0)
	end
	--Sets fps counter text
	DText.Text = string .. " fps"
end