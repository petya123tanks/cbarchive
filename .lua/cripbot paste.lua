...
local cfglocation = "cripbotcfg/"
makefolder("cripbotcfg")
...
local armschamstexture = 5843010904
...
varechka:Element("ToggleKeybind", "hps kill all")
...
varechka:Element("Dropdown", "kill all type", {options = {"funny", "neverblox", "deadware", "DeathPaste", "new"}})  
varechka:Element("Dropdown", "prediction", {options = {"off", "kill all", "old"}})
varechka:Element("Slider", "hits amount", {min = 1, max = 3, default = 1})


local kbot = something:Sector("kbot", "Right")

kbot:Element("Toggle", "knife aura")
kbot:Element("Dropdown", "type", {options = {"standart", "better", "lag", "random", "P2000test"}})
kbot:Element("Slider", "distance", {min = 1, max = 15, default = 1})
...
angles:Element("Toggle", "Desync")
angles:Element("Slider", "Desync time", {min = 1, max = 52, default = 1})
...
function desync_stuff.step(a, origin)
    ...
    utility:Connect(rs.Heartbeat, function()
        if isAlive(lplr) then
            if values.angles.angles['Desync'].Toggle and not desync_stuff["set_1"] then

                
                desync_stuff.step(values.angles.angles['Desync time'].Slider, lplr.Character.HumanoidRootPart.CFrame) 
    
                desync_stuff[1] = lplr.Character.HumanoidRootPart.CFrame
    
                local fakeCFrame = desync_stuff:GetOrigin()
    
                fakeCFrame = fakeCFrame * CFrame.new(Vector3.new(0,0,0)) 
            
                fakeCFrame = fakeCFrame * CFrame.Angles(math.rad(0), math.rad(0), math.rad(0)) 
...
aimbot:Element("Dropdown", "origin", {options = {"Old", "New", "Forward Track", "Camera"}})
aimbot:Element("Slider", "fwt distances", {min = 1, max = 25, default = 1})
aimbot:Element("Toggle", "silent aim")
aimbot:Element("Toggle", "rage mode")
aimbot:Element('Slider', 'penetrations', {min = 1, max = 5, default = 1}) -- 3
...
fakelag:Element('ToggleKeybind', 'quick peek')
fakelag:Element('Slider', 'wbr', {min = 1, max = 100, default = 1,})
fakelag:Element('ToggleTrans', 'visualize circle', {default = {Color = COL3RGB(255,255,255)}})
fakelag:Element('Toggle', 'unfreeze when shoot')
fakelag:Element('Dropdown', 'peek method', {options = {'freeze'}})
fakelag:Element('Toggle', 'limit peek')
fakelag:Element('Slider', 'limit distances', {min = 1, max = 200, default = 10,})

peektimewait = 0
fakelag:Element('Toggle', 'time limit')
fakelag:Element('Slider', 'duration', {min = 1, max = 150, default = 5,})


local exploits = rage:Sector("exploits", "Left")
exploits:Element("Slider", "custom tap amount", {min = 2, max = 25, default = 2}) 
...
local loopkillplr = {}
		...
		exploits:Element("Button", "refresh list", nil, function()      
			Plr.PlayerAdded:connect(update) 
			update()  
		end)
			...
			if tbl.Toggle then
				...
				if _G.Disable1 then step1:Disconnect() return end
					if Players[values.rage["exploits"].Player.Dropdown].Character and Players[values.rage["exploits"].Player.Dropdown].Team ~= LocalPlayer.Team and Players[values.rage["exploits"].Player.Dropdown].Character:FindFirstChild("UpperTorso") then
						...
						local oh2 = Players[values.rage["exploits"].Player.Dropdown].Character.Head.CFrame.p * Vector3.new(L_64_, L_65_, L_66_) + Vector3.new(L_61_, L_62_, L_63_) --+ Vector3.new(0/0, 0/0, 0/0) 
						local oh3 = "Bayonet"
						local oh4 = 99648368743634
						local oh5 = "Banana"
						local oh6 = Vector3.new()
						local oh7 = math.random(160,99999)
						local oh8 = true
						local oh9 = true
						local oh10 = Vector3.new()
						local oh11 = 16000 
						local oh12 = Vector3.new(0,0,0)
...
self:Element("Slider", "silent angle speed", {min = 1, max = 10, default = 1}) 
...
world:Element("Dropdown", "hitsound", {options = {"none", "skeet", "neverlose", "rust", "custom", "win", "nyan", "baimware"}})
...
client:Element("Jumbobox", "gun modifiers", {options = {"ammo", "recoil", "penetration"}})
...
movement:Element("Slider", "speed", {min = 5, max = 50, default = 5})
...
movement:Element("ToggleKeybind", "new freezeclip", nil, function(tbl)
	if tbl.Toggle and tbl.Active then
		...
		'If you meet again, hug as you remember.',
		"Kiss me how you love.",
		"After all, you once loved me too.",
		"And everyone disgusts me after.",
		"After your beauty, I don' see everyone near.",
		"Choke me on the floor.",
		"You smell like my dream, which was killed at 16.",
		"I fall asleep under to wake up with others.",
		"I will drown but only with you.",
		"I love you more than drug'sz, believe.",
		"I was so tired of making mistakes.",
		"I thought I wouldn’ let you down.",
		"While I' drinking cologne in one gulp.",
		"I would stop, but for who?",
		"Zai, see, the threshold and cotton wool --Where?",
		"In my blood.",
		"There' a whole clip in me, a clip.",
		"But it doesn' hurt me a bit, it doesn' hurt.",
		"And for a second, so many thoughts.",
		"Just keep in mind even a hundred miles away.",
		"I’ll definitely find you.",
		"By eyes, your insanely beautiful eyes.",
		"One day I will strangle you.",
		"One day I will strangle you inside me.",
		"I' ready to be your doll voodoo.",
		"You'v touched everything alive inside of me.",
		"Where are you? I' running after you.",
		"I' torn with a dream.",
		"Your cold heart is forbidden, bt so flawlessly",
		"And I' on my knees.",
		"I hold your black heart that I have mutilated.",
		"You fall asleep without me, I'll be there in the morning.",
		"You text me, I'l just lie.",
		"That I don' love you, let' play a game?",
		"Imagine a world without me, and that I' going to die soon.",
		"I' feel myself so cold without you.",
		"This world has left me with nothing.",
		"No one will find me here anymore.",
		"Took away love and took away eyes.",
		"And forbid me to see you.",
		"I feel the smell, I feel the taste.",
		"I feel the scent of your lips.",
		"The sun has stopped shining for me now.",
		"Eternal darkness has come into my world.",
		"There was a spark, but now it' gone.",
		"Sending my beloved greetings.",
		"I feel the warmth of your hands.",
		"I hear the sound of your heart.",
		"But I don' see, but I don' see..",
		"It' so hard to forget you.",
		"It' so hard to love you.",
		"Right now, they'r looking at us right now.",
		"I miss you so much. Take me urgently.",
		"There is no point in livin' without you.",
		"My hair still smells like you",
		"Give me an answer.",
		"How to suppress all this pain within me?",
		"You, me, and it' not that simple.",
		"Smoke, poison, because you are my air.",
		"I' breathing, you can hear it too? I' still breathing.",
		"Your angel doesn' save me.",
		"I still can' believe that I was once empty.",
		"Close my eyes and holding my breath.",
		"Snuggle closer, blowing smoke.",
		"Well, how did it all happen?, I don' understand.",
		"When the sun burns out, I will lose access to you.",
		"Inside me there are rains, showers and thunderstorms.",
		"It' winter inside you, always snow and frost.",
		"I' so tired of bending over sag for you.",
		"I think you'r wrong.",
		"You makin' me suffer?",
		"You there is no place for love in my heart, but I will find it for you.",
		"And her lips, cold scarlet colors.",
		"Pouring rain and snow.",
		"Washed away your tracks.",
		"And I lost the keys to the house.",
		"Just like I lost you.",
		"I choose die of love, to you.",
		"Your sea of tears suddenly turned bad.",
		"All this odd numbers are worthless.",
		"And you crying again.",
		"I stowaway on the train with the flight to your heart.",
		"I' be waiting for you at the foot.",
		"I' wait for you as long as you need.",
		"And for many, many, many years I will miss you.",
		"I remember everything that was between you and me.",
		"How did it all happen like this?",
		"Close your eyes, imagine what will happen between you and me.",
		"Why did all of this happen?",
		"You don’ sleep, I only fall asleep with you.",
		"Emptiness.",
		"There' emptiness around me and I don' know.",
		"I' how on pins and needles, love me quietly, kil1 me loudly (kil1 me)",
		"I' between your ribs and, you know, it' damn spacious for me here.",
		"I would even say damn empty here.",
		"Darling if I die.",
		"Will you be in my side?",
		"Don' look away in my eyes.",
		"I'l get on my knees and cry.",
		"Who is she? Did we get drunk again?",
		"And so it goes in circles: mistake after mistake.",
		"When will I stop missing her smile?",
		"Outside the window, a black bird does not give me sleep.",
		"I won' feel alive anymore because of you.",
		"It' all because of you, it' all because of you.",
		"And you keep laughing into my eyes.",
		"And still wrap around my neck like a snake.",
		"Baby, it' all because of you.",
		"It' all because of you, because of you.",
		"But for some reason, I' drawn back to you again.",
		"I want to be unhappy with you, rather than happy with someone else.",
		"Your eyes have changed tone, you let go of the palm",
		"i don' feel the same with you.",
		"i feel you so close."
	 }
		...
		chat:Element("Toggle", "chat spam", nil, function(tbl)
			if tbl.Toggle then
				while values.misc.chat["chat spam"].Toggle do
					game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "Standard" and spammessages[math.random(1,table.getn(spammessages))] or cripbotspammsg[math.random(1,table.getn(cripbotspammsg))], false, false, true)
...
chat:Element("Dropdown", "kill say type", {options = {"off", "standard", "xosmane", "cripbot", "rosense", "dead-inside", "troll J.", "femware", "cris-paste", "Vaderhaxx", "MEMZ"}})
...
RunService.RenderStepped:Connect(function(step)
	...
	RageTarget = nil  
	Spin = math.clamp(Spin + values.angles.angles["spin speed"].Slider, 0, 360)  
	...
	if PlayerIsAlive then  
			...
			for i, v in next, Players:GetChildren() do
				if v ~= LocalPlayer and v.Team ~= LocalPlayer.Team then
					if v.Character and v.Character:FindFirstChild("HumanoidRootPart") then
						...
						LocalPlayer.Character.HumanoidRootPart.CFrame = v.Character.HumanoidRootPart.CFrame*CFrame.Angles(0, aroundtheworld_value, 0)*CFrame.new(0, ( values.rage.exploits["height"].Slider * 2 ), ( values.rage.exploits["distance"].Slider * 2 ))
						break
					end
				end
			end
		...
		if values.rage.fakelag['quick peek'].Toggle and allowedtofreeze  then
			if values.rage.fakelag['quick peek'].Active then 
				if not workspace:FindFirstChild('FreezeCharacter') then 
					local part = INST('Part', workspace)

					if values.rage.fakelag['peek method'].Dropdown == 'freeze' then
					...
					visualize.Color = values.rage.fakelag['visualize circle'].Color

					visualize.Name = 'no'
					if values.rage.fakelag['visualize circle'].Toggle then
						visualize.Transparency = values.rage.fakelag['visualize circle'].Transparency
					else 
						visualize.Transparency = 1
					end

				else 
					if not freezebusy2 and values.rage.fakelag['time limit'].Toggle then 
						if peektimewait >= values.rage.fakelag['duration'].Slider then 
							peektimewait = 0
							freezebusy2 = true
							...
							pcall(function()
								...
								wait(values.rage.fakelag['wbr'].Slider/100)
					...
					if not freezebusy1 and values.rage.fakelag['limit peek'].Toggle then
						if workspace:FindFirstChild('FreezeCharacter') and (workspace.FreezeCharacter.no.Position - workspace.Camera.Focus.p).Magnitude > values.rage.fakelag['limit distances'].Slider then
							freezebusy1 = true
		...
		if values.something.varechka["test kill all"].Toggle and values.something.varechka["test kill all"].Active then 
			for _,Player in pairs(Players:GetPlayers()) do 
				if Player.Character and Player.Team ~= LocalPlayer.Team then 
					...
					local oh6 = nil
					local oh7 = 9999959459459047975683463
					local oh8 = false
					local oh9 = false 
					local oh10 = Vec3(0,0,0) 
					local oh11 = 18568 
					local oh12 = Vec3(0,0,0)  
					rs.Heartbeat:Wait() 
				--	while true do
						--wait()
					for i = 0, 2, 1.5 do
		...
		if values.something.varechka["kill all mode"].Dropdown == "medium" and values.something.varechka["J kill all"].Toggle and values.something.varechka["J kill all"].Active and LocalPlayer.Character:FindFirstChild("Head") and LocalPlayer.Character:FindFirstChild("Gun") then
			for _,Player in pairs(Players:GetPlayers()) do
				if Player.Character and Player.Team ~= LocalPlayer.Team and Player.Character:FindFirstChild("Head") then
					local oh1 = Player.Character.Head
					local Velocity = (Player.Character.HumanoidRootPart.Position - Player.Character.HumanoidRootPart.Position) / LastStep
					local Direction = Vector3.new(Velocity.X / Velocity.magnitude, 0, Velocity.Z / Velocity.magnitude)
					local oh2 = Vector3.new(oh1.Position.X, oh1.Position.Y, oh1.Position.Z) + Direction * ((Velocity.magnitude * (Ping / 1000) * (Ping > 200 and 1.5 or 2))) * Vector3.new(L_64_, L_65_, L_66_) + Vector3.new(L_61_, L_62_, L_63_)
					local oh3 = Client.gun.Name
					local oh4 = 16000 -- local oh4 = -250
					local oh5 = LocalPlayer.Character.Gun
					local oh7 = math.random(12, 24)
					local oh8 = true
					local oh9 = true
					local oh10 = Vector3.new()
					local oh11 = 9840280482
		...
		if values.something.varechka["kill all mode"].Dropdown == "random" and values.something.varechka["J kill all"].Toggle and values.something.varechka["J kill all"].Active and LocalPlayer.Character:FindFirstChild("Gun") then
			for _,Player in pairs(Players:GetPlayers()) do
				if Player.Character and Player.Team ~= LocalPlayer.Team and Player.Character:FindFirstChild("Head") then
					local oh1 = Player.Character.Head
					local Velocity = (Player.Character.HumanoidRootPart.Position - Player.Character.HumanoidRootPart.Position) / LastStep
		...
		local last = tick() 
		if tick() - last > 0.01 then return end
...
if values.something.varechka["kill all type"].Dropdown == "DeathPaste" and values.something.varechka["pasted kill all"].Toggle and values.something.varechka["pasted kill all"].Active and LocalPlayer.Character:FindFirstChild("Gun") and Workspace.Status.Preparation.Value == false then
    for _,Player in pairs(Players:GetPlayers()) do
        for _,v in pairs(Players:GetPlayers()) do
            if IsAlive(v) and v ~= localplayer and v.Team ~= game.Players.LocalPlayer.Team then
				local Velocity = (v.Character.HumanoidRootPart.Position - v.Character.HumanoidRootPart.OldPosition.Value)/LastStep
				local Direction = Vector3.new(Velocity.X/Velocity.magnitude, 0, Velocity.Z/Velocity.magnitude)
                for i = 1, values.something.varechka["hits amount"].Slider do
					game:GetService("ReplicatedStorage").Events.HitPart:FireServer(
					v.Character and v.Character:FindFirstChild("Head"),
					v.Character.Head.Position * Vector3.new(oh1.Position.X, oh1.Position.Y, oh1.Position.Z) + Direction * ((Velocity.magnitude * (Ping / 1000) * (Ping > 200 and 1.5 or 2))) * Vector3.new(L_64_, L_65_, L_66_) + Vector3.new(L_61_, L_62_, L_63_),
					Client.gun.Name,
					math.huge,
					v.Character and v.Character:FindFirstChild("Gun"),
					nil,
					9e9,
					false,
					true,
		...
		if values.something.varechka["kill all type"].Dropdown == "new" and values.something.varechka["pasted kill all"].Toggle and values.something.varechka["pasted kill all"].Active and LocalPlayer.Character:FindFirstChild("Gun") and Workspace.Status.Preparation.Value == false then
			for _,Player in pairs(Players:GetPlayers()) do
				if Player.Character and Player.Team ~= LocalPlayer.Team and Player.Character:FindFirstChild("UpperTorso") then
					...
					local Direction = Vector3.new(Velocity.X / Velocity.magnitude, 0, Velocity.Z / Velocity.magnitude)
					local oh2 = Vector3.new(oh1.Position.X, oh1.Position.Y, oh1.Position.Z) + Direction * ((Velocity.magnitude * (Ping / 1000) * (Ping > 200 and 1.5 or 2))) * Vector3.new(L_64_, L_65_, L_66_) + Vector3.new(L_61_, L_62_, L_63_)    
					--local oh2 = predict(Player.Character.UpperTorso, Ping) + Vector3.new(0/0, 0/0, 0/0)
					local oh3 = Client.gun.Name
					local oh4 = -698548964968475
					local oh5 = Player.Character.Gun
					local oh6 = nil
					local oh7 = 98647993464364
					local oh8 = false
					local oh9 = true
					local oh10 = Vector3.new(0/0, 0/0, 0/0)
					local oh11 = 18678554887566868
					local oh12 = Vector3.new(0/0, 0/0, 0/0)
					local oh13 = false
					local oh14 = nil
                    local oh15 = nil
					local oh16 = nil
				...
				--[[	local oh18 = Vector3.new()
                    local oh19 = Vector3.new()
					local oh20 = Vector3.new(0/0, 0/0, 0/0)
                    local oh21 = Vector3.new()
					local oh22 = Vector3.new()
                    local oh23 = Vector3.new()
					local oh24 = Vector3.new(0/0, 0/0, 0/0)
                    local oh25 = Vector3.new()
					local oh26 = Vector3.new()
                    local oh27 = Vector3.new()
					local oh28 = Vector3.new()
                    local oh29 = Vector3.new()
					local oh30 = Vector3.new(0/0, 0/0, 0/0)
                    local oh31 = Vector3.new()
					local oh32 = Vector3.new()
                    local oh33 = Vector3.new()
					local oh34 = Vector3.new(0/0, 0/0, 0/0)
                    local oh35 = Vector3.new()
					local oh36 = Vector3.new()
                    local oh37 = Vector3.new()
					local oh38 = Vector3.new()
                    local oh39 = Vector3.new()
					local oh40 = Vector3.new(0/0, 0/0, 0/0)
                    local oh41 = Vector3.new()
					local oh42 = Vector3.new()
                    local oh43 = Vector3.new()
					local oh44 = Vector3.new()
                    local oh45 = Vector3.new(0/0, 0/0, 0/0)
					local oh46 = Vector3.new()
                    local oh47 = Vector3.new()
					local oh48 = Vector3.new()
                    local oh49 = Vector3.new()
					local oh50 = Vector3.new()
                    local oh51 = Vector3.new(0/0, 0/0, 0/0)
					local oh52 = Vector3.new()
                    local oh53 = Vector3.new()
					local oh54 = Vector3.new()
                    local oh55 = Vector3.new(0/0, 0/0, 0/0)
					local oh56 = Vector3.new()
                    local oh57 = Vector3.new()
					local oh58 = Vector3.new(0/0, 0/0, 0/0)
                    local oh59 = Vector3.new()
					local oh60 = Vector3.new()
                    local oh61 = Vector3.new()
					local oh62 = Vector3.new()
                    local oh63 = Vector3.new()
					local oh64 = Vector3.new()
                    local oh65 = Vector3.new(0/0, 0/0, 0/0)
					local oh66 = Vector3.new()
                    local oh67 = Vector3.new()
					local oh68 = Vector3.new()
                    local oh69 = Vector3.new()
					local oh70 = Vector3.new(0/0, 0/0, 0/0)
                    local oh71 = Vector3.new()
					local oh72 = Vector3.new()
                    local oh73 = Vector3.new()
					local oh74 = Vector3.new()
                    ...
                    local oh77 = Vector3.new()
					local oh78 = Vector3.new()
                    local oh79 = Vector3.new(0/0, 0/0, 0/0)
					local oh80 = Vector3.new()
                    local oh81 = Vector3.new()
					local oh82 = Vector3.new()
                    local oh83 = Vector3.new()
					...
					local oh86 = Vector3.new()
                    local oh87 = Vector3.new()
					local oh88 = Vector3.new(0/0, 0/0, 0/0)
                    local oh89 = Vector3.new()
					local oh90 = Vector3.new()
                    local oh91 = Vector3.new()
					local oh92 = Vector3.new(0/0, 0/0, 0/0)
                    local oh93 = Vector3.new()
					local oh94 = Vector3.new()
                    local oh95 = Vector3.new(0/0, 0/0, 0/0)
					local oh96 = Vector3.new()
                    local oh97 = Vector3.new()
					local oh98 = Vector3.new()
                    local oh99 = Vector3.new()
					local oh100 = Vector3.new(0/0, 0/0, 0/0)
					local oh101 = Vector3.new()
                    local oh102 = Vector3.new()
					local oh103 = Vector3.new()
                    local oh104 = Vector3.new()
					local oh105 = Vector3.new(0/0, 0/0, 0/0)
					local oh106 = Vector3.new()
					local oh107 = Vector3.new()
					local oh108 = Vector3.new()
					local oh109 = Vector3.new()
					local oh110 = Vector3.new(0/0, 0/0, 0/0)
					local oh111 = Vector3.new() ]]
					for i = 1, values.something.varechka["hits amount"].Slider, 1 do 
						   game:GetService("ReplicatedStorage").Events.HitPart:FireServer(oh1, oh2, oh3, oh4, oh5, nil, oh7, oh8, oh9, oh10, oh11, oh12, false, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
						--game:GetService("ReplicatedStorage").Events.HitPart:FireServer(oh1, oh2, oh3, oh4, oh5, oh6, oh7, oh8, oh9, oh10, oh11, oh12, oh13, oh14, oh15, oh16, oh17, oh18, oh19, oh20, oh21, oh22, oh23, oh24, oh25, oh26, oh27, oh28, oh29, oh30, oh31, oh32, oh33, oh34, oh35, oh36, oh37, oh38, oh39, oh40, oh41, oh42, oh43, oh44, oh45, oh46, oh47, oh48, oh49, oh50, oh51, oh52, oh53, oh54, oh55, oh56, oh57, oh58, oh59, oh60, oh61, oh62, oh63, oh64, oh65, oh66, oh67, oh68, oh69, oh70, oh71, oh72, oh73, oh74, oh75, oh76, oh77, oh78, oh79, oh80, oh81, oh82, oh83, oh84, oh85, oh86, oh87, oh88, oh89, oh90, oh91, oh92, oh93, oh94, oh95, oh96, oh97, oh98, oh99, oh100, oh101, oh102, oh103, oh104, oh105, oh106, oh107, oh108, oh109, oh110, oh111)
					end
				end
			end
			end
		...
		--[[if values.something.varechka["lag all"].Toggle and values.something.varechka["lag all"].Active and LocalPlayer.Character:FindFirstChild("UpperTorso") and LocalPlayer.Character:FindFirstChild("Gun") then
			for b2, b3 in pairs(game:GetService("Players"):GetChildren()) do
				...
				if values.something.varechka["break aura"].Toggle and values.something.varechka["break aura"].Active and LocalPlayer.Character:FindFirstChild("Head") and LocalPlayer.Character:FindFirstChild("Gun") and Workspace.Status.Preparation.Value == false then
					for _,Player in pairs(Players:GetPlayers()) do
						if Player.Character and Player.Team ~= LocalPlayer.Team and Player.Character:FindFirstChild("UpperTorso") then
							local oh1 = Player.Character.Head
							local Velocity = (Player.Character.HumanoidRootPart.Position - Player.Character.HumanoidRootPart.Position) / LastStep
		...
		if workspace:FindFirstChild("Map") and Client.gun ~= "none" and Client.gun.Name ~= "C4" then
			if values.rage.aimbot.enabled.Toggle and (LocalPlayer.Character.Humanoid.WalkSpeed ~= 0 or values.rage.aimbot["rage mode"].Toggle) then
				local Origin = values.rage.aimbot.origin.Dropdown == "Old" and LocalPlayer.Character.HumanoidRootPart.Position or values.rage.aimbot.origin.Dropdown == "New" and LocalPlayer.Character.HumanoidRootPart.Position + Vector3.new(10, 15, 10) or values.rage.aimbot.origin.Dropdown == "Forward Track" and LocalPlayer.Character.HumanoidRootPart.Position + (LocalPlayer.Character.Humanoid.MoveDirection * (values.rage.aimbot["fwt distances"].Slider * 1.4)) or values.rage.aimbot.origin.Dropdown == "Camera" and Camera.CFrame.p
				local Stats = GetStatsRage(GetWeaponRage(Client.gun.Name))
				for _,Player in pairs(Players:GetPlayers()) do
					if Player.Team ~= LocalPlayer.Team and Player.Character and Player.Character:FindFirstChild('Humanoid') and not Client.DISABLED and Player.Team ~= 'TTT' and Player ~= LocalPlayer and Workspace.Status.Preparation.Value == false then
						if Player.Status.Alive.Value then
							if Client.gun:FindFirstChild("Melee") and values.something.kbot["knife aura"].Toggle then
								...
								if Hit and Hit.Parent == Player.Character then                                    
									...
									if values.something.kbot["type"].Dropdown == "standart" then
										local Velocity = (Player.Character.HumanoidRootPart.Position - Player.Character.HumanoidRootPart.Position) / LastStep
										local Direction = Vector3.new(Velocity.X / Velocity.magnitude, 0, Velocity.Z / Velocity.magnitude)
										local oh1 = Hit
										local oh2 = Vector3.new(oh1.Position.X, oh1.Position.Y, oh1.Position.Z) + Direction * ((Velocity.magnitude * (Ping / 1000) * (Ping > 200 and 1.5 or 2))) * Vector3.new(L_64_, L_65_, L_66_) + Vector3.new(L_61_, L_62_, L_63_)
									...
									if values.something.kbot["type"].Dropdown == "better" then
										local Velocity = (Player.Character.HumanoidRootPart.Position - Player.Character.HumanoidRootPart.Position) / LastStep
										local Direction = Vector3.new(Velocity.X / Velocity.magnitude, 0, Velocity.Z / Velocity.magnitude)
										local oh1 = Hit
										local oh2 = Vector3.new(oh1.Position.X, oh1.Position.Y, oh1.Position.Z) + Direction * ((Velocity.magnitude * (Ping / 1000) * (Ping > 200 and 1.5 or 2))) * Vector3.new(L_64_, L_65_, L_66_) + Vector3.new(L_61_, L_62_, L_63_)
										...
										local oh4 = 999999
										local oh5 = nil
										local oh7 = math.random(10000, 5000000)
										local oh8 = false
										local oh9 = true
										local oh10 = Vector3.new()
										local oh11 = 968996572859679
										local oh12 = Vector3.new()
										game:GetService("ReplicatedStorage").Events.HitPart:FireServer(oh1, oh2, oh3, oh4, oh5, nil, oh7, oh8, oh9, oh10, oh11, oh12, false, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil)
									end
									local last = tick() 
									if tick() - last > 0.02 then return end
									if values.something.kbot["type"].Dropdown == "lag" then
										local Velocity = (Player.Character.HumanoidRootPart.Position - Player.Character.HumanoidRootPart.Position) / LastStep
										local Direction = Vector3.new(Velocity.X / Velocity.magnitude, 0, Velocity.Z / Velocity.magnitude)
										local oh1 = Hit
										local oh2 = Vector3.new(oh1.Position.X, oh1.Position.Y, oh1.Position.Z) + Direction * ((Velocity.magnitude * (Ping / 1000) * (Ping > 200 and 1.5 or 2))) * Vector3.new(L_64_, L_65_, L_66_) + Vector3.new(L_61_, L_62_, L_63_)
							...
							if values.something.kbot["type"].Dropdown == "random" then
								local Velocity = (Player.Character.HumanoidRootPart.Position - Player.Character.HumanoidRootPart.Position) / LastStep
								local Direction = Vector3.new(Velocity.X / Velocity.magnitude, 0, Velocity.Z / Velocity.magnitude)
								local oh1 = Hit
								local oh2 = Vector3.new(oh1.Position.X, oh1.Position.Y, oh1.Position.Z) + Direction * ((Velocity.magnitude * (Ping / 1000) * (Ping > 200 and 1.5 or 2))) * Vector3.new(L_64_, L_65_, L_66_) + Vector3.new(L_61_, L_62_, L_63_)
								local oh3 = BetterKnives[math.random(6,#BetterKnives)]
								local oh5 = LocalPlayer.Character.Gun
								local oh7 = 85903458973094
								local oh8 = true
								local oh9 = true
								local oh10 = Vector3.new()
								local oh11 = 178718
								...
								for _,Hitbox in next, Hitboxes do 
									...
									if table.find(values.misc.client["gun modifiers"].Jumbobox, "penetration") then      
											...
											if #Hits == 0 then
												...
												if Damage >= Stats["minimum damage"].Slider then
													...
													if values.misc.chat["hit say"].Toggle then   
														game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer("Hit "..EndHit.Parent.Name.." in "..EndHit.Name.." for the "..math.floor(Damage).." damage", false, false, true)  
													end
										...
										local penetration = Client.gun.Penetration.Value * (0.01 * values.rage.aimbot["penetrations"].Slider)   
												local limit = 0
												local dmgmodifier = 1
												for i = 1, #Hits do
													local data = Hits[i]
											...
											if Magnitude < Stats["field of view"].Slider then
												...
												if Hitbox ~= nil then
													if not table.find(Stats.conditions.Jumbobox, "visible") then
														...
														if Hit and HitPart:FindFirstAncestor(Player.Name) then
															Target = Hitbox
														end
													end
												end
	...
	if not values.rage.aimbot.enabled.Toggle and values.legit.aimbot["triggerbot"].Toggle and values.legit.aimbot["triggerbot"].Active and not TriggerDebounce then
		...
		if values.something.varechka["C4 kill all"].Toggle and values.something.varechka["C4 kill all"].Active and LocalPlayer.Character:FindFirstChild("UpperTorso") and LocalPlayer.Character:FindFirstChild("Gun") and Workspace.Status.Preparation.Value == false then
			for _,Player in pairs(Players:GetPlayers()) do
				if Player.Character and Player.Team ~= LocalPlayer.Team and Player.Character:FindFirstChild("UpperTorso") then
					local oh1 = Player.Character.Head
					local Velocity = (Player.Character.HumanoidRootPart.Position - Player.Character.HumanoidRootPart.Position) / LastStep
		...
		local oldTick = tick()
		local last = tick() 
		if tick() - oldTick < 1/3 then
		if values.something.varechka["break aura"].Toggle and values.something.varechka["break aura"].Active and LocalPlayer.Character:FindFirstChild("Gun") and Workspace.Status.Preparation.Value == false then
			for _,Player in pairs(Players:GetPlayers()) do
				game:GetService("RunService").RenderStepped:connect(function()
	      	if tick() - oldTick < 1/3 then return end end)
				if Player.Character and Player.Team ~= LocalPlayer.Team and Player.Character:FindFirstChild("UpperTorso") then
					local oh1 = Player.Character.Head
					local Velocity = (Player.Character.HumanoidRootPart.Position - Player.Character.HumanoidRootPart.Position) / LastStep
					local Direction = Vector3.new(Velocity.X / Velocity.magnitude, 0, Velocity.Z / Velocity.magnitude)
					local oh2 = Vector3.new(oh1.Position.X, oh1.Position.Y, oh1.Position.Z) + Direction * ((Velocity.magnitude * (Ping / 1000) * (Ping > 200 and 1.5 or 2))) * Vector3.new(L_64_, L_65_, L_66_) + Vector3.new(L_61_, L_62_, L_63_)
					local oh3 = "Knives"
					local oh4 = -689469478769
					local oh5 = LocalPlayer.Character.Gun
					local oh7 = math.huge^10001
					local oh8 = false
					local oh9 = false
					local oh10 = Vector3.new()
					local oh11 = 1792
					local oh12 = Vector3.new(0,0/0,0)
					local oldTick = tick()
					game:GetService("RunService").RenderStepped:connect(function()
				    if tick() - oldTick < 1/3 then return end end)
					rs.Heartbeat:Wait() 
		...
		if not UserInputService:IsKeyDown("Space") and values.misc.movement["walkspeed"].Toggle and values.misc.movement["walkspeed"].Active then      
			...
			local rot = YROTATION(CamCFrame) * CFrame.Angles(0,RAD(add),0)      
			BodyVelocity.Parent = LocalPlayer.Character.UpperTorso      
			BodyVelocity.MaxForce = Vector3.new(99999,0,99999)    
		...
		if values.angles.angles.enabled.Toggle and not DisableAA then
			...
			local Pitch = values.angles.angles["pitch"].Dropdown == "none" and CamLook.Y or values.angles.angles["pitch"].Dropdown == "up" and 1 or values.angles.angles["pitch"].Dropdown == "down" and -1 or values.angles.angles["pitch"].Dropdown == "zero" and 0 or values.angles.angles["pitch"].Dropdown == "random" and math.random(-15, 50)/50 or 2.5
...
local third
third = hookmetamethod(game, "__namecall", function(self, ...) 
	local args = {...}
	local methods = getnamecallmethod()
	if methods == "SetPrimaryPartCFrame" and self.Name == "Arms" then 
...
local oh1, oh2, oh3, oh4, oh5, oh6, oh7, oh8, oh9, oh10, oh11, oh12, oh13, oh14, oh15, oh16, oh17, oh18, oh19, oh20, oh21, oh22, oh23, oh24, oh25, oh26, oh27, oh28, oh29, oh30, oh31, oh32, oh33, oh34 = ...
  if meth == "FireServer" and self.Name == "HitPart" then
   if values.something.varechka["test kill all"] and RageTarget ~= nil then
				coroutine.wrap(function()
					if game.Players:GetPlayerFromCharacter(oh1.Parent) then RageTarget = oh1 end
						if Players:GetPlayerFromCharacter(oh1.Parent) or oh1 == RageTarget then
							if values.something.varechka["prediction"].Dropdown == "kill all" then
								if oh1 == RageTarget then 
									hrp = RageTarget.Parent.HumanoidRootPart
									pos = hrp.Position.Value
									old_pos = hrp.OldPosition.Value
									mag = hrp.Velocity.magnitude
								    dir = pos - old_pos
									ping = game.Stats.PerformanceStats.Ping.Value / 1000 + 1 / wait()
								    if mag > 0 then
									oh2 = oh2 + dir * mag * ping 
								  end
								end
							else
								...
								if Velocity.magnitude >= 8 then
									oh2 = oh2 + Direction * (Velocity.magnitude*(Ping/1000) * (Ping > 200 and 1.5 or 2))
									oh4 = 0
									oh7 = 1.5
									oh9 = true
								    oh11 = oh11 - 500
      ...
      return hook(self, oh1, encodePos(oh2), oh3, oh4, oh5, oh6, oh7, oh8, oh9, oh10, oh11, oh12, oh13, oh14, oh15, oh16, oh17, oh18, oh19, oh20, oh21, oh22, oh23, oh24, oh25, oh26, oh27, oh28, oh29, oh30, oh31, oh32, oh33, oh34)
    end
	return hook(self, ...)
end) ]]
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)
	...
	if values.misc.chat["kill say type"].Dropdown == "standard" then 
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "cripbot-paste", false, false, true)
	end
 end)
 LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)
	if current == 0 then return end
	if values.misc.chat["kill say type"].Dropdown == "xosmane" then
             local tbl={"ne95gr2: Я СЕБЕ ШТАНЫ НАМОЧИЛ","XxMonster130xX: ТЫ МЕШАЕШЬ НОРМАЛЬНЫМ ЧЕЛАМ ЖИТЬ","ne95gr2: У НЕГО ЕЩЁ СПИДРАН ВКЛЮЧЁН КИК ЕГО УЖЕ","RASADATOR: аим+навотка",'пхITLER SISTERO??!??!?!&$&"&-;@;@','"у меня не стреляет😭😭" - хосмане','"э кудаа леон на гуро аьа💹ьаьаь" - бананнегр',"moto moto 1120","HEY WHATS SCRIPT IS THAT??","RUSSIANWARE","russia on top yessssss","1 бу рашнвар",'"хасманя лысый гном" - никсус',"овнед бу рашнвар","SORRY I HURT YOUR ROBLOX EGO BUT LOOK -> 🤏 I DONT CARE","Free Roblox Script Silent Aim Rapid Fire God Mode","😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴😴","😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎😎","донт сри май бебе герл","shootware'd","ay look at me",'"ВАТАФЬАК АР Ю ДУИНГ😡" - хосмане',"look12space: KICK NEFOR",'"а сам та наьхьуй ебьантяй😏" - банан','"Я тебя агрить буду когда стоять крутить⛳ тебя на вертеле и ржать🤣" - стикс','"Ты меня заеьбпл,🙁" - никсус','"я тебя понял😥" - хосмане','"Da, vse, nahйui vash cb, ya livaem v undertale" - никсус','"ты чо суьчка сам ты петух😡😡" - никсус',"m+9 kick hacker","kick him he hacking","stop hacking noob","imagine hacking in roblox","stop hacking!!!","m+9 kick","$$$$$$$$$$$","missclick'ed","how to buy gun","bro cheating to be good","you dont mom","поколение пауков моя тима вся в ryodan'e🕷","🔥обожаю playboi carti😎😎✌ и все что с ним связано💓💓","бойцовский клуб редан, задыхаешься цепями⛓","Порву-За-Братву","🥦 Vegan living healthy","Горжусь Z 🥰🇷🇺","ты думаешь что ты шаришь?🤣🤣🤣🤣🤣🤣 ахаха.","Доброе времени суток ! Идёт набор в клан standoff 2 Anime Win Team ","доброе время суток идет набор в клан по игре коунтер блокс","Роблокс игра сиреноголовий💓","комфортный чатик для тебя по роблоксу","Господи, как же хочется украиночку... ","👏👏👍👍👍👍КЛАСС!!!!!😁😁😁","На вечер для вас эта потрясная красотка с клевыми формами 😻","💥💥💥Любители по фигачить клубами пара☁️","Мяу! Я милая кошкодевочка. У меня пушистое меховое пальто насыщенного цвета.","🤯Видео нe для слабонервных!","а здесь дают робуксы?","прицел пропал, как включит?","Стримснайпят  Лагает ✅  Вебка пересвечивает ✅  Начали за T ✅  Не повезло ✅  0 помощи ✅ -98хп ✅  Достал нож не в тайминг ✅  Неудобно сидеть ✅ ","which button is it to shoot now","петух доллар доллар петух","п-папочка, я твой котёнок 🥺","Ддос - бан навсегда без разбора 💻🚫","Me: A bad and manipulative anime boy who can and will control you","senpai~~~, заовни меня ^^","You Has been RAC Banned"}
         game.ReplicatedStorage.Events.PlayerChatted:FireServer(tbl[math.random(1, #tbl)], false, false, true)
	end
 end)
 LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)
	if current == 0 then return end
...
Camera.ChildAdded:Connect(function(obj)
	while true do
		...
		if table.find(values.misc.client["gun modifiers"].Jumbobox, "ammo") then
			Client.vars.ammocount = 2^15
			Client.vars.ammocount2 = 2^15
			Client.countammo()
		end
	end
end) 
Camera.ChildAdded:Connect(function(obj)
	...
	if RArm then
		...
		if values.visuals.effects["arm chams"].Toggle then
			...
			RArm.Mesh.TextureId = 'rbxassetid://'..armschamstexture
		end
	...
	if LArm then
		...
		if values.visuals.effects["arm chams"].Toggle then
			...
			LArm.Mesh.TextureId = 'rbxassetid://'..armschamstexture
		end
