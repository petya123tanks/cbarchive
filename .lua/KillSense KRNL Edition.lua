...
function library:New(name) 
	...
	TextLabel.Position = UDIM2(0, 266, 0, 0) 
	TextLabel.Size = UDIM2(0, 0, 0, 29) 
	TextLabel.Size = UDIM2(0, txt:GetTextSize(name, 5, Enum.Font.RobotoMono, Vec2(720, TextLabel.AbsoluteSize.Y)).X, 1.8, 100) 
...
for _,lua in pairs(listfiles("killlua")) do 
	local luaname = GSUB(lua, "killlua\\", "") 
	INSERT(allluas, luaname) 
end 

RunService.RenderStepped:Wait() 

local gui = library:New("KillSense") 
...
luascripts:Element("Button", "load", {}, function() 
	loadstring(readfile("killlua\\"..values.luas["lua scripts"].lua.Scroll))() 
end) 
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do 
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "Winning" and "Join the winning team. Join KillSense!" or "$$$ I'm using KillSense lol $$$", false, "Innocent", false, true) 
			wait(values.misc.chat["speed (ms)"].Slider/1000) 
			 
		end 
	end 
end) 
chat:Element("Dropdown", "type", {options = {"Winning", "KillSense"}}) 
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current) 
	...
	if values.misc.chat["kill say"].Toggle then 
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "#Winning with KillSense" and values.misc.chat["message"].Text or "Allah is the Man", false, "Innocent", false, true) 
	end 
end) 
