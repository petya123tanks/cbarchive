...
local cfglocation = "sexwarecfg/"      
makefolder("sexwarecfg")      
...
function library:New(name)      
	...
	Menu.Image = "http://www.roblox.com/asset/?id=8107819009"      
			...
			function MSector:Tab(text)      
				...
				function tab:Element(type, text, data, callback)      
					...
					if type == "Jumbobox" then      
						...
						ImageLabel.Image = "http://www.roblox.com/asset/?id=8107819009"      
...
makefolder("sexwarelua")      


RunService.RenderStepped:Wait()      

local gui = library:New("SEXWARE.CC VERSION 0.0.1")      
...
configs:Element("Button", "Watermark", nil, function()
	...
	library:CreateWatermark("SEXWARE.CC | COMPLETELY THE BEST | VERSION: 0.0.1")
...
chat:Element("Toggle", "chat spam", nil, function(tbl)      
	if tbl.Toggle then      
		while values.misc.chat["chat spam"].Toggle do      
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "hi" or "GET GOOD. GET SEXWARE", false, "Innocent", false, true)      
...
grenades:Element("Button", "crash server", {}, function() 
	...
	while true do
        pcall(function()
            game:GetService("RunService").RenderStepped:Wait()
            for i = 1,100,1 do																																																																									-- Server Crasher exploit by property (SEXWARE). mis hermanos solo usan SEXWARE
...
CreateHitElement("Loading SEXWARE..",Color3.new(1,1,0))
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)      
	...
	if values.misc.chat["kill say"].Toggle then      
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "GET REKT BY SEXWARE.CC", false, "Innocent", false, true)      
	end      
end)      
