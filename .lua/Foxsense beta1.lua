	...
	library:CreateWatermark("Foxsense BETA")
	...
	local gui = library:New("Foxsense")      
	local legit = gui:Tab("Legit")      
	local rage = gui:Tab("Rage")      
	local visuals = gui:Tab("Visuals")      
	local misc = gui:Tab("Misc")    
	local grief = gui:Tab("Server-Side")      
	...
	antiaim:Element("Dropdown", "pitch", {options = {"none", "up", "down","huge", "custom", "zero", "180", "random", "Random AA"}})   
	...
	world:Element("Dropdown", "hitsound", {options = {"none", "skeet", "neverlose", "rust", "bag", "baimware","cod","minecraft","godlike","fard"}})      
	world:Element("Slider", "sound volume", {min = 1, max = 5, default = 3})      
	world:Element("Dropdown", "skybox", {options = {"none", "nebula", "vaporwave", "clouds", "twilight"}}, function(tbl)      
	...
	chat:Element("Toggle", "chat spam", nil, function(tbl)      
		if tbl.Toggle then      
			while values.misc.chat["chat spam"].Toggle do      
				game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "Im using Foxsense!" or "Join Foxsense for more update!", false, "Innocent", false, true)      
				wait(values.misc.chat["speed (ms)"].Slider/1000)      
					
			end      
		end      
	end)      
	chat:Element("Dropdown", "type", {options = {"standard", "Foxsense$$$"}})      
	...
	CreateHitElement("Loading Bloxrium...",Color3.new(1,1,0))
	...
	RunService.RenderStepped:Connect(function(step)     
		...
		if PlayerIsAlive then      
									...
									for _,Hitbox in ipairs(Hitboxes) do      
										...
										if values.rage.aimbot["automatic penetration"].Toggle then      
											...
											if Hit and Multipliers[Hit.Name] ~= nil then      
												...
												Damage = Damage * (Client.gun.RangeModifier.Value/110 ^ ((Origin - Hit.Position).Magnitude/500))      
												if Damage >= Stats["minimum damage"].Slider then      
													RageGuy = Hit      
													RageTarget = Hit      
													if not values.rage.aimbot["silent aim"].Toggle then      
			...
			if values.rage.angles.enabled.Toggle and not DisableAA then      
				...
				if values.rage.angles["extend pitch"].Toggle and (values.rage.angles["pitch"].Dropdown == "up" or values.rage.angles["pitch"].Dropdown == "down") or values.rage.angles["pitch"].Dropdown == "Random AA" and 148 or values.rage.angles["pitch"] then      
					Pitch = (Pitch*2)/1.6      
				end      
	...
	LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)      
		...
		if values.misc.chat["kill say"].Toggle then      
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "1'd by bloxrium" and values.misc.chat["message"].Text or "2 ez for bloxrium" and values.misc.chat["message"].Text or "beamed by bloxrium", false, "Innocent", false, true)      
		end      
	end)      
	...
	CreateHitElement("Loaded Bloxrium.OOO!!",Color3.new(0,1,0))