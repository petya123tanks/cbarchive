...
local r6_dummy = game:GetObjects("rbxassetid://10521168220")[1]

r6_dummy.Parent = workspace.Ray_Ignore
r6_dummy.Name = ""
r6_dummy.Humanoid:Destroy()
Instance.new("AnimationController", r6_dummy).Name = "Humanoid"
r6_dummy:SetPrimaryPartCFrame(CFrame.new(Vector3.new(1,1,1) * 99e99))
...
r6_dummy = {ins = r6_dummy, playing = {}}

function r6_dummy.stop(id)
    if r6_dummy.playing[id] then
        r6_dummy.playing[id].Stop(r6_dummy.playing[id])
    end
end

function r6_dummy.play(id, ...)
    r6_dummy.stop(id)

    local animation = Instance.new("Animation")
    animation.AnimationId = tostring(id)

    local loaded = r6_dummy.ins.Humanoid.LoadAnimation(r6_dummy.ins.Humanoid, animation)
    loaded.Play(loaded, ...)

    r6_dummy.playing[id] = loaded
end

function r6_dummy.goto(a)
    r6_dummy.ins:SetPrimaryPartCFrame(a)
end

function r6_dummy.update()
...
function library:New(args)
    ...
    local name = args.name or args.Name or "BARAN"
    local accent1 = args.accent1 or args.Accent1 or Color3.fromRGB(102, 255, 102)
    ...
    local window_frame = utility:Draw("Image", nil, {
    ...
    function window:Toggle()
        ...
        if window_frame.Visible then
            cas:UnbindAction("barankeyboard")
            cas:UnbindAction("baranwheel")
            cas:UnbindAction("baranm1")
            cas:UnbindAction("baranm2")
            ...
            cas:BindAction("barankeyboard", function() end, false, Enum.UserInputType.Keyboard)
            cas:BindAction("baranwheel", function() end, false, Enum.UserInputType.MouseWheel)
            cas:BindAction("baranm1", function() end, false, Enum.UserInputType.MouseButton1)
            cas:BindAction("baranm2", function() end, false, Enum.UserInputType.MouseButton2)
    ...
    function window:Watermark()
        local watermark = {name = "baran", version = "dev", instances = {}, values = {}}
...
-- заебали-- заебали-- заебали-- заебали-- заебали-- заебали-- заебали-- заебали-- заебали-- заебали-- заебали-- заебали-- заебали-- заебали

local window = library:New({name = "BARAN | DEV VERSION 1 | SO BAD BUT COOL 🤍💙❤"})
...
local visuals_effects = visuals:Section({name = "Effects", side = "right"})
local visuals_selfeffects = visuals:Section({name = "Self Effects", side = "left"})
local visuals_worldeffects = visuals:Section({name = "World Effects", side = "right"})

local misc_movement = misc:Section({name = "Movement", side = "right"})
local misc_clieent = misc:Section({name = "Client settings", side = "left"})
local misc_trolling = misc:Section({name = "Troll 🤣🤣🤣", side = "left"})

local rage_aimbot = rage:Section({name = "Aimbot", side = "left"})
local rage_fwtandbt = rage:Section({name = "Forward track and Backtrack", side = "right"})
local rage_antiaim = rage:Section({name = "Anti-aim", side = "right"})
local rage_knifebot = rage:Section({name = "Knifebot", side = "left"})
local rage_autobuy = rage:Section({name = "Auto-buy", side = "right"})
...
local exploits_baran = exploits:Section({name = "Baran's things", side = "left"})
...
do
    visuals_esp:Toggle({name = "Esp", flag = "esp_enabled"}):Keybind()
    ...
    visuals_effects:Label({name = "Hitsound", middle = true})
    visuals_effects:Toggle({name = 'Enabled', flag = 'hitsound_enabled'})
    visuals_effects:Dropdown({name = "Hitsounds", options = indexListing(hitsounds), flag = "hitsound_flag"})
    visuals_effects:Slider({name = "Volume", min = 1, max = 10, default = 5, ending = "%", flag = "hitsound_vol"})
    
    visuals_selfeffects:Toggle({name = "Third person", flag = "s_tper"})
    visuals_selfeffects:Slider({name = "Third person distance", min = 10, max = 30, default = 15, decimals = 0.5, ending = "studs", flag = "s_tper_distance"})
    visuals_selfeffects:Toggle({name = "Viewmodels", flag = "view_enabled"})
    visuals_selfeffects:Slider({name = "X", min = -180, max = 180, default = 0, ending = "°", flag = "view_x"})
    visuals_selfeffects:Slider({name = "Y", min = -180, max = 180, default = 0, ending = "°", flag = "view_y"})
    visuals_selfeffects:Slider({name = "Z", min = -180, max = 180, default = 0, ending = "°", flag = "view_z"})
    visuals_selfeffects:Slider({name = "Roll", min = -180, max = 180, default = 0, ending = "°", flag = "view_roll"})
    visuals_worldeffects:Toggle({name = "No Kill Feed", flag = "ot_nokf"})
    visuals_worldeffects:Toggle({name = "No gun bob", flag = "ot_no_bob"})
    visuals_worldeffects:Label({name = "Global effects", middle = true})
    visuals_worldeffects:Toggle({name = "Enabled", flag = "enabled_worldflag"})
    visuals_worldeffects:Toggle({name = "Better Shadow", flag = "betshadflag", callback = function(state)
        sethiddenproperty(game.Lighting, "Technology", library.flags["enabled_worldflag"] and state and "ShadowMap" or "Legacy")
    end})
    visuals_worldeffects:Colorpicker({name = "Ambient", flag = "OutDooram_colorflag", def = Color3.fromRGB(255, 255, 255), callback = function(val)
    ...
    visuals_worldeffects:Colorpicker({name = "Tint", flag = "tintcolor_colorflag", def = Color3.fromRGB(255, 255, 255), callback = function(val)
        workspace.CurrentCamera.ColorCorrection.TintColor = library.flags["enabled_worldflag"] and val or Color3.new(1, 1, 1)
    end})
    visuals_worldeffects:Colorpicker({name = "Fog", flag = "fogcolor_flag", def = Color3.fromRGB(255, 255, 255), callback = function(val)
    ...
    visuals_worldeffects:Slider({name = "Time Changer", min = 10, max = 30, default = 15, decimals = 0.5, ending = "hr", flag = "time_flag", callback = function(state)
...
do
    ...
    exploits_desync:Toggle({name = "Enabled", flag = "desync_enabled"})
    exploits_desync:Slider({name = "Frames offset", min = 0, max = 360, default = 0, flag = "desync_frames_offset"})
    exploits_desync:Dropdown({name = "Mode", options = {"-", "Offset", "Random", "Invisible"}, flag = "desync_mode"})
    exploits_desync:Dropdown({name = "Rotation", options = {"Manual", "Random"}, flag = "desync_rotation"})
    exploits_desync:Toggle({name = "No footstep sound", flag = "no_footstep"})
    exploits_hitbox_desync:Label({name = "Offset mode", middle = true})
    exploits_hitbox_desync:Slider({name = "Offset X", min = -10, max = 10, def = 0, suf = "st", flag = "desync_offset_x"})
    exploits_hitbox_desync:Slider({name = "Offset Y", min = -10, max = 10, def = 0, suf = "st", flag = "desync_offset_y"})
    exploits_hitbox_desync:Slider({name = "Offset Z", min = -10, max = 10, def = 0, suf = "st", flag = "desync_offset_z"})
    exploits_hitbox_desync:Label({name = "Random mode", middle = true})
    exploits_hitbox_desync:Slider({name = "Random X", min = 0, max = 35, def = 10, suf = "st", flag = "desync_random_x"})
    exploits_hitbox_desync:Slider({name = "Random Y", min = 0, max = 35, def = 10, suf = "st", flag = "desync_random_y"})
    exploits_hitbox_desync:Slider({name = "Random Z", min = 0, max = 35, def = 10, suf = "st", flag = "desync_random_z"})
    exploits_hitbox_desync:Label({name = "Manual rotation", middle = true})
    exploits_hitbox_desync:Slider({name = "Manual X", min = -180, max = 180, def = 0, suf = "°", flag = "desync_manual_x"})
    exploits_hitbox_desync:Slider({name = "Manual Y", min = -180, max = 180, def = 0, suf = "°", flag = "desync_manual_y"})
    exploits_hitbox_desync:Slider({name = "Manual Z", min = -180, max = 180, def = 0, suf = "°", flag = "desync_manual_z"})
    exploits_hitbox_desync:Button({name = "Reset", callback = function()
...
do
    ...
    misc_movement:Toggle({name = "Bhop", flag = "m_bhop_enabled"})
    misc_movement:Slider({name = "Speed", min = 15, max = 300, default = 60, ending = "studs/s", flag = "m_bhop_speed"})
    misc_movement:Toggle({name = "CFrame walkspeed", flag = "m_cfr_ws_enabled"})
...
do
    ...
    misc_trolling:Toggle({name = "Spam chat"})
    misc_trolling:Toggle({name = "Kill say"})
    misc_trolling:Toggle({name = "Respawn", callback = function()
        local ReplicatedStorage = game:GetService("ReplicatedStorage")
        ReplicatedStorage:FindFirstChild("Events").Spawnme:FireServer()
        end})
...
do
    ...
    rage_aimbot:Toggle({name = "Enabled", flag = "rage_enabled"})
    ...
    rage_aimbot:Dropdown({name = "Select", options = {"Ammo", "Firerate", "Recoil"}, def = {}, multi = true, flag = "gunmods"})
    ...
    rage_aimbot:Label({name = "Kill all", middle = true})
    rage_aimbot:Toggle({name = "Enabled", flag = "kall_enabled"})
    rage_aimbot:Slider({name = "Hits per run", min = 1, max = 60, default = 5, ending = " hit(s)", flag = "kall_hpr"})
    rage_aimbot:Toggle({name = "On preparation", flag = "kall_prep"})

    rage_fwtandbt:Label({name = "Forward track", middle = true})
    rage_fwtandbt:Toggle({name = "Forward track", flag = "rage_ftrack"})
    rage_fwtandbt:Slider({name = "Forward track amount", min = 0.5, max = 15, default = 1.5, decimals = 0.5, flag = "rage_ftrack_amount"})
    rage_fwtandbt:Label({name = "BackTrack", middle = true})
    rage_fwtandbt:Toggle({name = "Backtrack", flag = "rage_btrack"})
    rage_fwtandbt:Slider({name = "Backtrack offset", min = 0, max = 360, default = 0, ending = " frames", flag = "rage_backtrack"})
    
    rage_knifebot:Toggle({name = "Enabled", flag = "kbot_enabled"})
    rage_knifebot:Toggle({name = "Auto equip", default = true, flag = "kbot_equip"})
    ...
    rage_antiaim:Toggle({name = "Enabled", flag = "aa_enabled"})
    ...
    rage_autobuy:Toggle({name = "Enabled", flag = "abuy_enabled"})
    rage_autobuy:Dropdown({name = "Primary page", options = {"Heavy", "SMGS", "Rifles"}, flag = "abuy_page"})
    rage_autobuy:Dropdown({name = "Primary", options = {"-", "1", "2", "3", "4", "5", "6"}, flag = "abuy_primary"})
    rage_autobuy:Dropdown({name = "Secondary", options = {"-", "1", "2", "3", "4", "5"}, flag = "abuy_secondary"})
...
do
    ...
    settings_config:SubButtons({buttons = {
        {"Save", function()
            writefile("baran/universal/configs/" .. library.flags["config_name"] .. ".baran", window:GetConfig())
        end},
        {"Load", function()
            if isfile("baran/universal/configs/" .. library.flags["config_selected"] .. ".baran") then
                window:LoadConfig(readfile("baran/universal/configs/" .. library.flags["config_selected"] .. ".baran"))
            end
        end}
    }})
    ...
    settings_game:Button({name = "Rejoin", callback = function()
    ...
    oldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
        ...
        if library.loaded then
            ...
            elseif (method == "Play" or method == "Stop") and self.ClassName == "AnimationTrack" then
                if method == "Play" then
                    r6_dummy.play(self.Animation.AnimationId, unpack(args))
                else
                    r6_dummy.stop(self.Animation.AnimationId)
                end
            end
        end
