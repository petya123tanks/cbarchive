...
local MainUIColor = COL3RGB(30, 155, 255)    
...
function library:New(name)      
	...
	TextLabel.BackgroundColor3 = COL3RGB(117, 2, 255)  
	...
	TabButtons.BackgroundColor3 = COL3RGB(117, 2, 255)  
...
local gui = library:New("Stormy.cc v1")      
...
aimbot:Element("ToggleKeybind", "monkeybot")      
...
local function AddLegit(Tab)      
	...
	Tab:Element("Slider", "field of view", {min = 30, max = 860, default = 120})      
...
aimbot:Element("Slider", "prediction hitchance", {min = 0, max=100, default = 50})
...
local function AddRage(Tab)      
	...
	Tab:Element("Slider", "minimum damage", {min = -250, max = 100, default = 20})      
	Tab:Element("Slider", "max fov", {min = 1, max = 420, default = 180})      
end      
...
movement:Element("Slider", "speed", {min = 0, max = 500, default = 40})      
...
RunService.RenderStepped:Connect(function(step)     
	...
	if PlayerIsAlive then      
		...
		if workspace:FindFirstChild("Map") and Client.gun ~= "none" and Client.gun.Name ~= "C4" then      
			...
			if not values.rage.aimbot.enabled.Toggle and values.legit.aimbot["monkeybot"].Toggle and values.legit.aimbot["monkeybot"].Active and not TriggerDebounce then      
				local Stats = GetStatsLegit(GetWeaponLegit(Client.gun.Name))      
				if Stats.monkeybot.Toggle then      
					if not TBLFIND(Stats.conditions.Jumbobox, "blind") or LocalPlayer.PlayerGui.Blnd.Blind.BackgroundTransparency > 0.9 then      
						if not TBLFIND(Stats.conditions.Jumbobox, "standing") or SelfVelocity.Magnitude < 3 then      
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)      
	...
	if values.misc.chat["kill say"].Toggle then      
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "😎😎 被中国黑客窃听 😎😎", false, "Innocent", false, true)  
	end      
end)      
