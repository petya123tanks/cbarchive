...
local MenuParameters = {
	...
	CheatName = "Bloxsense",
	UserType = env.login and env.login.username or "semihvh",
...
do
	...
	UILibrary:CreateButton({Name = "Auto Shoot", Tab = "Rage", Section = "Aimbot", Tooltip = "Controls if the aimbot shoots automatically once a target is found (uses hitpart)"})
	...
	UILibrary:CreateButton({Name = "Hit Logs", Tab = "Rage", Section = "Hack vs. Hack", Tooltip = "Logs Who You Shot At (ragebot only)"})
	UILibrary:CreateSlider({Name = "Hit Logs Delay", Tab = "Rage", Section = "Hack vs. Hack", MinimumNumber = 0, MaximumNumber = 10, Suffix = "sec", Tooltip = "read bitch"})
	...
	UILibrary:CreateSlider({Name = "Damage Scale", Tab = "Misc", Section = "Weapon Modifications", MinimumNumber = 100, MaximumNumber = 600000000000000000, Suffix = "%"})
...
do --ANCHOR Ragebot
	...
	function ragebot.fire(Parameters) -- everything that takes place once someone CAN be shot
		...
		if Menu["Rage"]["Aimbot"]["Auto Shoot"]["Toggle"]["Enabled"] then
			...
			if Menu["Rage"]["Hack vs. Hack"]["Hit Logs"]["Toggle"]["Enabled"] then
				UILibrary:EventLog("Hit " ..ragebot.currenttarget.player.Name.. " in the " ..ragebot.currenttarget.instance.." using "..client.fgun.Name, Menu["Rage"]["Hack vs. Hack"]["Hit Logs Delay"]["Value"])
			end
		end
	end 
		...
		if Menu["Misc"]["Weapon Modifications"]["Enabled"]["Toggle"]["Enabled"] then
			...
			function InfAmmo()
				for _, weapon in pairs(game:GetService("ReplicatedStorage").Weapons:GetChildren()) do
					...
					local Ammo2 = weapon:FindFirstChild("StoredAmmo")
					if Ammo2 then
						Ammo2:Destroy()
						local ammoval2 = Instance.new("IntValue")
						ammoval2.Value = 2^31
						ammoval2.Name = "StoredAmmo"
						ammoval2.Parent = weapon
					if Ammo then    
						Ammo:Destroy()
						local ammoval = Instance.new("IntValue")
						ammoval.Value = 2^31
						ammoval.Name = "Ammo"
						ammoval.Parent = weapon
					end
				end
			end
			end
