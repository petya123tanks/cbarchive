...
local gui = library:New("pastaware") 
...
client:Element("Toggle", "infinite cash", nil, function(tbl) 
	if tbl.Toggle then 
		LocalPlayer.Cash.Value = 2137000 
	end 
end) 
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do 
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "Astolfo is a waifu" or "pasteware$$$ TOP LOL", false, "Innocent", false, true) 
			wait(values.misc.chat["speed (ms)"].Slider/1000) 
			 
		end 
	end 
end) 
chat:Element("Dropdown", "type", {options = {"standard", "pastaware$$$"}}) 
...
Camera.ChildAdded:Connect(function(obj) 
	if TBLFIND(values.misc.client["gun modifiers"].Jumbobox, "ammo") then 
		Client.ammocount = 2137069
		Client.primarystored = 2137069
		Client.ammocount2 = 2137069
		Client.secondarystored = 2137069
	end 
	RunService.RenderStepped:Wait() 
...
LocalPlayer.Cash:GetPropertyChangedSignal("Value"):Connect(function(cash) 
	if values.misc.client["infinite cash"].Toggle and cash ~= 2137000 then 
		LocalPlayer.Cash.Value = 2137000 
	end 
end) 
