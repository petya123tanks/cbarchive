...
local cfglocation = "cloudycfg/"  
makefolder("cloudycfg")  
...
antiaim:Element("Slider", "yaw offset", {min = -400, max = 400, default = 0})  
antiaim:Element("Toggle", "jitter")  
antiaim:Element("Slider", "jitter offset", {min = -400, max = 400, default = 0})  
antiaim:Element("Dropdown", "pitch", {options = {"none", "up", "down", "zero", "180", "Skeet", "random"}})  
antiaim:Element("Toggle", "extend pitch")  
antiaim:Element("Dropdown", "body roll", {options = {"off", "180", "wtf", "90"}})  
...
self:Element("Slider", "viewmodel y", {min = -15, max = 15}, function(val)  
	ViewmodelOffset = CF(values.visuals.self["viewmodel x"].Slider/7, values.visuals.self["viewmodel y"].Slider/7, values.visuals.self["viewmodel z"].Slider/7) * CFAngles(0, 0, values.visuals.self.roll.Slider/50)  
end)  
...
client:Element("Toggle", "infinite cash", nil, function(tbl)  
	if tbl.Toggle then  
		LocalPlayer.Cash.Value = 99999999999999 
	end  
end)  

local D19ForniteCardToggle = false

client:Element("Toggle", "19 dollar fornite card", nil, function()  
...
chat:Element("Toggle", "chat spam", nil, function(tbl)  
	if tbl.Toggle then  
		while values.misc.chat["chat spam"].Toggle do  
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "hi" or "get stormy.lol", false, "Innocent", false, true)  
...
chat:Element("Slider", "speed (ms)", {min = 0, max = 1000, default = 500})  
...
ui:Element("Slider", "amount", {min = 5, max = 13, default = 10})  
...
RunService.RenderStepped:Connect(function(step)  
	...
	if PlayerIsAlive then  
      ...
      if values.rage.angles["body roll"].Dropdown == "wtf" then  
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "wtf" and RAD(213) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = 3
			else  
				LocalPlayer.Character.Humanoid.HipHeight = 2  
			end  

      if values.rage.angles["body roll"].Dropdown == "90" then  
				Root.CFrame = Root.CFrame * CFAngles(values.rage.angles["body roll"].Dropdown == "wtf" and RAD(90) or 0, 1, 0)  
				LocalPlayer.Character.Humanoid.HipHeight = 3
			else  
				LocalPlayer.Character.Humanoid.HipHeight = 2  
			end  

			local Pitch = values.rage.angles["pitch"].Dropdown == "none" and CamLook.Y or values.rage.angles["pitch"].Dropdown == "up" and 1 or values.rage.angles["pitch"].Dropdown == "down" and -1 or values.rage.angles["pitch"].Dropdown == "zero" and 0 or values.rage.angles["pitch"].Dropdown == "random" and RANDOM(-10, 10)/10 or 2.5 or values.rage.angles["pitch"].Dropdown == "Skeet" and -1
