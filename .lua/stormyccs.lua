...
local MainUIColor = COL3RGB(255, 192, 203)    
...
function library:New(name)      
	...
	do 
		...
		TextLabel.BorderColor3 = COL3RGB(255, 192, 203) 
	...
	TextLabel.BackgroundColor3 = COL3RGB(255, 192, 203)  
	...
	TextLabel.Text = name  
	TextLabel.TextColor3 = COL3RGB(255, 192, 203)  
	TextLabel.TextSize = 13.000  
	TextLabel.TextXAlignment = Enum.TextXAlignment.Left  

	TabButtons.Name = "TabButtons"  
	TabButtons.Parent = Menu  
	TabButtons.BackgroundColor3 = COL3RGB(255, 192, 203)  
	...
	Tabs.Name = "Tabs"  
	Tabs.Parent = Menu  
	Tabs.BackgroundColor3 = COL3RGB(255, 192, 203)  
	...
	function menu:Tab(text)  
		...
		TabGui.Name = "TabGui"  
		TabGui.Parent = Tabs  
		TabGui.BackgroundColor3 = COL3RGB(255, 192, 203)  
		...
		Left.Name = "Left"  
		Left.Parent = TabGui  
		Left.BackgroundColor3 = COL3RGB(255, 192, 203)  
		...
		Right.Name = "Right"  
		Right.Parent = TabGui  
		Right.BackgroundColor3 = COL3RGB(255, 192, 203)  
...
local gui = library:New("Uwusense")      
...
antiaim:Element("Dropdown", "pitch", {options = {"none", "up", "down", "Among", "180", "funny", "cc", "cc2", "cc3", "random", "God", "fucker", "cool", "cool2", "back"}})  
...
local Uwusensexyz = Instance.new("ScreenGui")
local Main = Instance.new("TextButton")
local Frame = Instance.new("Frame")
    

Uwusensexyz.Name = "Uwusense.xyz"
Uwusensexyz.Parent = game.CoreGui
Uwusensexyz.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

    
Main.Size = UDim2.new(0,200,0,40)
Main.Transparency = 1
Main.Parent = Uwusensexyz
...
CreateHitElement("Loading Uwusense.xyz..",Color3.new(1,1,0))
...
griefsector:Element("Toggle","show team damage",{},function(tbl)
	...
	Uwusensexyz.Enabled = TeamDamage
	spawn(function()
		while TeamDamage do
			pcall(function()
				local UIListLayout = Instance.new("UIListLayout")
...
RunService.RenderStepped:Connect(function(step)     
	...
	if PlayerIsAlive then      
      ...
      if values.rage.angles["body roll"].Dropdown == "-262" then  
			...
			local Pitch = values.rage.angles["pitch"].Dropdown == "none" and 0 or values.rage.angles["pitch"].Dropdown == "up" and 1 or values.rage.angles["pitch"].Dropdown == "down" and -1 or values.rage.angles["pitch"].Dropdown == "Among" and 15 or values.rage.angles["pitch"].Dropdown == "funny" and 20 or values.rage.angles["pitch"].Dropdown == "cc" and math.huge -5/0 -1 or values.rage.angles["pitch"].Dropdown == "cc2" and 23 or values.rage.angles["pitch"].Dropdown == "cool2" and -13 or values.rage.angles["pitch"].Dropdown == "cool" and 12 or values.rage.angles["pitch"].Dropdown == "back" and -92 or values.rage.angles["pitch"].Dropdown == "God" and 15 or values.rage.angles["pitch"].Dropdown == "fucker" and -13 or values.rage.angles["pitch"].Dropdown == "random" and RANDOM(-100, 100)/10 or 2.5  
			if values.rage.angles["extend pitch"].Toggle and (values.rage.angles["pitch"].Dropdown == "up" or values.rage.angles["pitch"].Dropdown == "down" or values.rage.angles["pitch"].Dropdown == "Among") then  
				Pitch = (Pitch*2)/1.6  
			end  
...
repeat wait() until game:IsLoaded() local genv = getgenv() local screen = Instance.new("ScreenGui") syn.protect_gui(screen) screen.Parent = game.CoreGui local bg = Instance.new("Frame",screen) bg.Size = UDim2.new(0,0,0,18) bg.BorderColor3 = Color3.new(0,0,0) local g = Instance.new("UIGradient",bg) g.Color = ColorSequence.new{ColorSequenceKeypoint.new(0,Color3.fromRGB(65,65,65)); ColorSequenceKeypoint.new(1,Color3.fromRGB(35,35,35));} g.Rotation = 90; local h = Instance.new("Frame",bg) h.Size = UDim2.new(1,0,0,1) h.BorderSizePixel = 1 h.BorderColor3 = Color3.new(0,0,0) local tl = Instance.new("TextLabel",bg) tl.Size = UDim2.new(.96,0,.92,0) tl.Position = UDim2.new(.02,0,.095,0) tl.BackgroundTransparency = 1 tl.TextStrokeTransparency = 0 tl.TextColor3 = Color3.new(1,1,1) tl.TextStrokeColor3 = Color3.new(0,0,0) tl.TextSize = 8 genv.watermark = {text = {"[Uwusense] Cuteware.xyz";"[UID: 1]";game.Players.LocalPlayer.Name.." | Build: Beta";"00:00:00";};color = Color3.fromRGB(255, 192, 203), separator = "  |  "} game:GetService("RunService").Heartbeat:Connect(function() watermark.text[4] = os.date("%X",os.time()) tl.Text = table.concat(watermark.text,watermark.separator) local x = game:GetService("TextService"):GetTextSize(tl.Text,tl.TextSize,tl.Font,Vector2.new(0,0)).X local screensize = workspace.CurrentCamera.ViewportSize bg.Size = UDim2.new(0,x+12,0,20) bg.Position = UDim2.new(0,screensize.X-25-x,0,25) h.BackgroundColor3 = watermark.color end)

ChatScript.moveOldMessages()

ChatScript.createNewMessage("Uwusense","Build: Private",MainUIColor,Color3.new(1,1,1),0.01,nil)

ChatScript.moveOldMessages()

ChatScript.createNewMessage("Uwusense","Version: 0.1.3",MainUIColor,Color3.new(1,1,1),0.01,nil)	
