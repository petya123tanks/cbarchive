...
function library:Tween(...) TweenService:Create(...):Play() end
  local cfglocation = "barancfgs/"
  makefolder("barancfgs")
  ...
  function library:New(name)
    ...
    local niggerssszz = INST("ScreenGui")
    ...
    niggerssszz.Name = "niggerssszz"
    niggerssszz.ResetOnSpawn = false
    niggerssszz.ZIndexBehavior = "Global"
    niggerssszz.DisplayOrder = 420133769

    local UIScale = INST("UIScale")
    UIScale.Parent = niggerssszz
    ...
    but.Parent = niggerssszz

    local cursor = INST("ImageLabel")
    cursor.Name = "cursor"
    cursor.Parent = niggerssszz
    ...
    cursor.Visible = niggerssszz.Enabled
    cursor.Position = UDIM2(0,Mouse.X-32,0,Mouse.Y-28)
    end)

    Menu.Name = "Menu"
    Menu.Parent = niggerssszz
    Menu.BackgroundColor3 = COL3RGB(0, 0, 0)
    Menu.Position = UDIM2(0.5, -300, 0.5, -300)
    Menu.Size = UDIM2(0, 600, 0, 700)
    Menu.Image = "rbxassetid://14854782566"
    Menu.ImageTransparency = 0.9
    ...
    if key.KeyCode == Enum.KeyCode.Insert then
      niggerssszz.Enabled = not niggerssszz.Enabled
      library.uiopen = niggerssszz.Enabled
    end
    end)
    ...
    function menu:Tab(text)
      ...
      TextButton.Size = UDIM2(0, txt:GetTextSize(text, 15, Enum.Font.GothamBold, Vec2(700,700)).X+12, 1, 0)
      ...
      if currenttab ~= text then
        ...
        library:Tween(TextButton, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = COL3RGB(0, 150, 0)})
        currenttab = text
        TabGui.Visible = true
      end
      end)
      ...
      function Tab:MSector(text, side)
        ...
        function MSector:Tab(text)
          ...
          function tab:Element(type, text, data, callback) -- NAM 
            local Element = {}
            data = data or {}
      ...
      function Tab:Sector(text, side)
		...
		TextBox.BorderColor3 = COL3RGB(33, 33, 33)      
        ...
        elseif type == "Label" then -- SUDA
          Section.Size = Section.Size + UDIM2(0,0,0,40)
          Element.value = {}

          local Label = INST("Frame")
          local TextLabel = INST("TextLabel")

          Label.Name = "Label"
          Label.Parent = Inner
          Label.BackgroundColor3 = COL3RGB(255, 255, 255)
          Label.BackgroundTransparency = 1.000
          Label.Position = UDIM2(0, 0, 0.542059898, 0)
          Label.Size = UDIM2(1, 0, 0, 30)

          TextLabel.Parent = Label
          TextLabel.BackgroundColor3 = COL3RGB(20, 20, 20)
          TextLabel.BackgroundTransparency = 1.000
          TextLabel.Position = UDIM2(0.108303241, 0, 0.224465579, 0)
          TextLabel.Size = UDIM2(0, 175, 0, 20)
          TextLabel.Font = Enum.Font.GothamBold
          TextLabel.Text = text
          TextLabel.TextColor3 = COL3RGB(255, 255, 255)
          TextLabel.TextSize = 14.000

          function SetValue(dag)
          dog = dag
          print("dog")
          end
        elseif type == "Dropdown" then
  ...
  niggerssszz.Parent = game.CoreGui
...
repeat RunService.RenderStepped:Wait() until game:IsLoaded()
  ...
  repeat wait() until Models ~= nil
    ...
    repeat wait() until ChrModels ~= nil
      ...
      for _,lua in pairs(listfiles("bloxlua")) do
        local luaname = GSUB(lua, "bloxlua\\", "")
        INSERT(allluas, luaname)
      end

      RunService.RenderStepped:Wait()
      local gui = library:New("baran")
...
local bublik = misc:Sector("Vajnoe nahuy", "Left")
local dayn = true
bublik:Element("Button", "3D Render", {}, function()
...
bublik:Element("Button", "Rejoin", nil, function()
...
bublik:Element("Button", "FPS Booster", {}, function()
...
-- русская рулетка для шторми паст скачиваем

bublik:Element("Button", "Русская рулетка (f9)", {}, function()

	local rdan = math.random(10)

	print("сегодня ты ахуенчик или нет... XDD ща узнаем лОл")
	task.wait(1)
	print(".")
	task.wait(1)
	print("..")
	...
	print("...")
	task.wait(0.5)
	print("анаLизируем информацию... 5 либо 10 очков те выпало...)")
	task.wait(5)
	if rdan > 5 then
	print(rdan, "XDDD ЕБАТЬ ТЫ ЛОХ")
		task.wait(5)
    game.Workspace:Detroy()
	elseif rdan <= 5 then
	print(rdan, "поvеzло, поробуй ещё раz)))")
	end
end)
...
aimbot:Element("ToggleKeybind", "Rage")
...
--[[if values.rage.aimbot["backtrack"].Toggle then
    ...
    for _,hitbox in pairs(game.Players:GetPlayers()) do
        if hitbox:IsA("BasePart") and hitbox.Name ~= "HumanoidRootPart" then
        local nigga = Instance.new("Part")
        nigga.CFrame = hitbox.CFrame
        nigga.Anchored = true
        nigga.CanCollide = false
        nigga.Material = Enum.Material.ForceField
        nigga.Color = Color3.fromRGB(255, 255, 255)
        nigga.Name = hitbox.Name
        nigga.Transparency = 0
        nigga.Size = hitbox.Size
        nigga.Parent = HexagonFolder

        local BacktrackTag = Instance.new("ObjectValue")
        BacktrackTag.Parent = nigga
        BacktrackTag.Name = "PlayerName"
        BacktrackTag.Value = v

        spawn(function()
            wait(0.1*(values.rage.aimbot["backtrack ms"].Slider))
        nigga:Destroy()
        end)
    else
...
aimbot:Element("Slider", "backtrack ms", {min = 0, max = 100, default = 1})   
aimbot:Element("Slider", "penetration", {min = 0, max = 100, default = 1})
aimbot:Element("Jumbobox", "resolver", {options = {"pitch", "arms", "roll", "animation"}})
aimbot:Element("Slider", "hitbox extender", {min = 0, max = 1000, default = 1})
...
exploits:Element("ToggleKeybind", "Damage+")
exploits:Element("ToggleKeybind", "Damage-")
...
local desync = rage:Sector("desync", "Right")
local Signal = loadstring(game:HttpGet("https://raw.githubusercontent.com/Quenty/NevermoreEngine/version2/Modules/Shared/Events/Signal.lua"))()
...
desync:Element("ToggleKeybind", "Enabled")
desync:Element("Toggle", "Fling")
desync:Element("Toggle", "Visualize")
desync:Element("Dropdown", "Mode", {options = {"-", "Offset", "Random", "Zero"}})
desync:Element("Dropdown", "Rotation", {options = {"Manual", "Random"}})
desync:Element("Label", "Offset mode")
desync:Element("Slider", "Offset X", {min = -1000, max = 1000, default = 0})
desync:Element("Slider", "Offset Y", {min = -1000, max = 1000, default = 0})
desync:Element("Slider", "Offset Z", {min = -1000, max = 1000, default = 0})
desync:Element("Label", "Random mode")
desync:Element("Slider", "Random X", {min = -1000, max = 1000, default = 0})
desync:Element("Slider", "Random Y", {min = -1000, max = 1000, default = 0})
desync:Element("Slider", "Random Z", {min = -1000, max = 1000, default = 0})
desync:Element("Label", "Manual rotation")
desync:Element("Slider", "Manual X", {min = -1000, max = 1000, default = 0})
desync:Element("Slider", "Manual Y", {min = -1000, max = 1000, default = 0})
desync:Element("Slider", "Manual Z", {min = -1000, max = 1000, default = 0})
desync:Element("Button", "Reset")
...
function utility:Draw(class, offset, properties, hidden)
    ...
    if isAlive(lplr) then
        if values.rage.desync["Enabled"].Toggle and not desync_stuff["set_1"] then
            local fling = values.rage.desync.Fling.toggle
            desync_stuff[1] = lplr.Character.HumanoidRootPart.CFrame
            if values.rage.desync["Fling"].Toggle then
            ...
            if values.rage.desync["Mode"].Dropdown == "Offset" then
                fakeCFrame = fakeCFrame * CFrame.new(Vector3.new(values.rage.desync["Offset X"].Slider, values.rage.desync["Offset Y"].Slider, values.rage.desync["Offset Z"].Slider))
            elseif  values.rage.desync["Mode"].Dropdown == "Random" then
                fakeCFrame = fakeCFrame * CFrame.new(RandomVectorRange(values.rage.desync["Random X"].Slider, values.rage.desync["Random Y"].Slider, values.rage.desync["Random Z"].Slider))
            elseif  values.rage.desync["Mode"].Dropdown == "Zero" then
                fakeCFrame = CFrame.new(nan, nan, nan)
            end

            if values.rage.desync["Rotation"].Dropdown == "Manual" then
                fakeCFrame = fakeCFrame * CFrame.Angles(math.rad(values.rage.desync["Manual X"].Slider), math.rad(values.rage.desync["Manual Y"].Slider), math.rad(values.rage.desync["Manual Z"].Slider))
            elseif values.rage.desync["Rotation"].Dropdown == "Random" then
            ...
            if values.rage.desync["Visualize"].Toggle then
    ...
    if desync_stuff[1] and desync_stuff["set_1"] and isAlive(lplr) then
        ...
        if desync_stuff[2] and values.rage.desync["Fling"].Toggle and desync_stuff["set_2"] then
...
self:Element("Slider", "X", {min = -100, max = 100}, function(val)
ViewmodelOffset = CF(values.visuals.self["X"].Slider/7, values.visuals.self["Y"].Slider/7, values.visuals.self["Z"].Slider/7) * CFAngles(0, 0, values.visuals.self.roll.Slider/50)
end)
self:Element("Slider", "Y", {min = -100, max = 100}, function(val)
ViewmodelOffset = CF(values.visuals.self["X"].Slider/7, values.visuals.self["Y"].Slider/7, values.visuals.self["Z"].Slider/7) * CFAngles(0, 0, values.visuals.self.roll.Slider/50)
end)
self:Element("Slider", "Z", {min = -100, max = 100}, function(val)
ViewmodelOffset = CF(values.visuals.self["X"].Slider/7, values.visuals.self["Y"].Slider/7, values.visuals.self["Z"].Slider/7) * CFAngles(0, 0, values.visuals.self.roll.Slider/50)
end)
...
world:Element("Toggle", "bullet Tracers",  nil, function(tbl)
		if tbl.Toggle then
			local ess = true
			   if ess == true then
				local Services = {
...
world:Element("Toggle", "Asus wood")
world:Element("Slider", "Transparency", {min = 0, max = 100, default = 0})
world:Element("Dropdown", "hitsound", {options = {"none", "skeet", "neverlose", "rust", "bag", "baimware", "MC" , "ding"}})
world:Element("Slider", "sound volume", {min = 1, max = 5, default = 3})

local dddd = Instance.new("ColorCorrectionEffect", workspace.CurrentCamera)
...
client:Element("Toggle", "Music kit 1#", nil, function(tbl)
if tbl.Toggle then
		Musickit = game:GetService("RunService").RenderStepped:Connect(function()
			pcall(function()
            if values.misc.client["Music kit 1#"].Toggle then
				    game.StarterGui.Music.ValveCT.Win.SoundId = "rbxassetid://14311887593"
				    game.StarterGui.Music.ValveT.Win.SoundId = "rbxassetid://14311887593"
				    game.StarterGui.Music.ValveCT.Lose.SoundId = "rbxassetid://15073446652"
            game.StarterGui.Music.ValveT.Lose.SoundId = "rbxassetid://15073446652"
            game.StarterGui.Music.ValveCT.Bomb.SoundId = "rbxassetid://15073449057"
            game.StarterGui.Music.ValveT.Bomb.SoundId = "rbxassetid://15073449057"
				end
			end)
		end)
	else
...
if tbl.Toggle then
LocalPlayer.Cash.Value = "Baranware"
end
end)
...
client:Element("Toggle", "spectators list", nil, function(tbl)      
	if tbl.Toggle then      
        ...
        Players.ZIndex = 4
        Players.Font = Enum.Font.GothamBold
    ...
    local speclist = game.CoreGui.SpectatorsList
    speclist:Destroy()
   end     
end)
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "russia" and "discord.gg/q756adyBqp | lunar suck my dick L to cactuss XD" or "😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂😂", false, "Innocent", false, true)
			wait(values.misc.chat["speed (ms)"].Slider/100)
		end
	end
end)
chat:Element("Dropdown", "type", {options = {"russia", "emoji"}})
chat:Element("Slider", "speed (ms)", {min = 0, max = 100, default = 50})
...
local function CreateHitElement(text,col,time, size, size2, size3, size4) 
    spawn(function()
        ...
        library:Tween(Grad, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1.000})  
        library:Tween(Label, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1.000})
...
CreateHitElement("",Color3.new(255,255,255), 5, 0, 500,  0, 22)
CreateHitElement("Loading...",Color3.new(255,255,255), 5, 0, 500,  0, 22)
CreateHitElement("Loading...",Color3.new(255,255,255), 5, 0, 500,  0, 22)
CreateHitElement("Loading...",Color3.new(255,255,255), 5, 0, 500,  0, 22)
CreateHitElement("",Color3.new(255,255,255), 5, 0, 500,  0, 22)
CreateHitElement("CreateHitElement V",Color3.new(0,150,0), 5, 0, 500,  0, 22)
...
if tbl.Toggle then
game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer("!!!", false, "Innocent", false, true)
end
spawn(function()
...
CreateHitElement("",Color3.new(255,255,255), 5, 0, 500,  0, 22)
CreateHitElement("Animations dance yo V",Color3.new(0,150,0), 5, 0, 600,  0, 22)
...
if values.rage.aimbot["Rage"].Toggle then
            ...
            if Hit and Multipliers[Hit.Name] ~= nil then
              ...
              if Damage >= Stats["minimum damage"].Slider then
                ...
                if values.rage.aimbot["automatic fire"].Dropdown == "standard" then
                  ...
                  CreateHitElement(" "..EndHit.Parent.Name.." | "..EndHit.Name.." | "..math.floor(Damage).." ",Color3.new(1,1,1), 5, 0, 300,  0, 22) 
                elseif values.rage.aimbot["automatic fire"].Dropdown == "hitpart" then
                  CreateHitElement(" "..EndHit.Parent.Name.." | "..EndHit.Name.." | "..math.floor(Damage).." ",Color3.new(1,1,1), 5, 0, 300,  0, 22) 
                  Client.firebullet()
                  local Arguments = {
...
if not values.rage.aimbot["Rage"].Toggle and values.legit.aimbot["triggerbot"].Toggle and values.legit.aimbot["triggerbot"].Active and not TriggerDebounce then
local Stats = GetStatsLegit(GetWeaponLegit(Client.gun.Name))
...
CreateHitElement("",Color3.new(255,255,255), 5, 0, 500,  0, 22)
CreateHitElement("Rage V",Color3.new(0,150,0), 5, 0, 500,  0, 22)
CreateHitElement("Pitches V",Color3.new(0,150,0), 5, 0, 500,  0, 22)
CreateHitElement("Kill all V",Color3.new(0,150,0), 5, 0, 500,  0, 22)
CreateHitElement("Double tap V",Color3.new(0,150,0), 5, 0, 500,  0, 22)
CreateHitElement("Legit V",Color3.new(0,150,0), 5, 0, 500,  0, 22)
CreateHitElement("Anti aim V",Color3.new(0,150,0), 5, 0, 500,  0, 22)
CreateHitElement("",Color3.new(255,255,255), 5, 0, 500,  0, 22)
CreateHitElement("Esp V",Color3.new(0,150,0), 5, 0, 500,  0, 22)
...
if method == "FireServer" then
    if LEN(self.Name) == 38 then
        return
    elseif self.Name == "FallDamage" and TBLFIND(values.misc.client["damage bypass"].Jumbobox, "fall") then
        return
...
if values.rage.exploits["Damage+"].Toggle then
            args[8] *= 99999999 * 99999999 * 99999999
end
if values.rage.exploits["Damage-"].Toggle then
  args[8] *= -99999999 * -99999999 * -99999999
end
...
if (values.rage.aimbot["baranka"].Toggle and RageTarget ~= nil) then
    coroutine.wrap(function()
    if Players:GetPlayerFromCharacter(args[1].Parent) or args[1] == RageTarget then
        local hrp = RageTarget.Parent.HumanoidRootPart.Position
        local oldHrp = RageTarget.Parent.HumanoidRootPart.OldPosition.Value
...
CreateHitElement("Hitchams V",Color3.new(0,150,0), 5, 0, 500,  0, 22)
CreateHitElement("Impacts V",Color3.new(0,150,0), 5, 0, 500,  0, 22)
CreateHitElement("Damage + - V",Color3.new(0,150,0), 5, 0, 500,  0, 22)
CreateHitElement("",Color3.new(255,255,255), 5, 0, 500,  0, 22)
CreateHitElement("Visuals V",Color3.new(0,150,0), 5, 0, 500,  0, 22)
...
CreateHitElement("Hit sounds V",Color3.new(0,150,0), 5, 0, 500,  0, 22)

--[[local killsaytable = {
    "Горный баран — крупное животное плотного телосложения с высокими тонкими ногами, коротким хвостом.",
    "Горные бараны живут группами до 100 животных.",
    "У баранов и овец прямоугольные зрачки, впрочем, как и  у мангустов,  и осьминогов.",
    "Есть такие породы баранов, которые стоят около 2 000 000 долларов.",
    "У баранов и овец  есть удивительная способность запоминать своего пастуха.",
    "Бараны – коренастые копытные млекопитающие с четырьмя ногами, толстым туловищем и коротким хвостом. Эти животные имеют раздвоенные посередине копыта.",
    "По данным National Geographic, культовые изогнутые рога барана могут весить до 13,5 кг, что «больше, чем все кости в его теле вместе взятые».",
    "Различают семь подвидов баранов на основе количества хромосом (от 52 до 58) и строения митохондриальной ДНК.",
    "Бараньи стада разделены по половому признаку.",
    "Цвет шерсти однотонный и варьируется от белой и сероватой до коричневой и даже полностью черной.",
    "Вокруг шеи у баранов имеется своеобразная грива, а под основным мехом растет подшерсток.",
    "Снежные бараны умеют ходить на цыпочках. Эти горные обитатели привыкли к пересеченной и каменистой местности.",
    "Дикие бараны не агрессивны, но их пугает присутствие людей.",
    "Баран - это копытные млекопитающие, что делает их основной мишенью для крупных хищников во всех частях света.",
    "Эти животные могут бегать со скоростью 32 км/ч. Они также могут поддерживать скорость 24 км/ч при подъеме в гору.",
    "Минимальное ежедневное потребление пищи составляет около 0,067 кг травы или сена на каждые килограмм веса тела.",
    "Бараны относятся к типу травоядных, известных как жвачные.",
    "Дикие бараны водятся почти во всех горных регионах мира.",
	"Бараны являются одними из самых популярных сельскохозяйственных животных.",
	"Когда вы смотрите на маленький кончик бараньего рога, вы видите весь рог, который был у барашка в молодости.",
	"Бараны живут в среднем 10-12 лет.",
	"Бараны – дневные животные, но в жаркую погоду они могут сменить образ жизни, отдыхая днем в тенистых местах и кормясь вечером или даже ночью.",
	"Как и люди, бараны имеют молочные зубы, которые они теряют с возрастом.",
	"Бараны – пасущиеся животные."
} ]]
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current) 
	if current == 0 then return end 
	if values.misc.chat["kill say"].Toggle then 
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text) 
	end 
end) 

CreateHitElement("Kill say V",Color3.new(0,150,0), 5, 0, 500,  0, 22)
...
if values.visuals.world["Asus wood"].Toggle then
	for _, obj in pairs(workspace.Map.Regen:GetDescendants()) do
		if obj:IsA("BasePart") and obj.Material == Enum.Material.Wood then
			if obj.Transparency ~= values.visuals.world["Transparency"].Slider/100 then
				obj.Transparency = values.visuals.world["Transparency"].Slider/100
			end
		end
	end
...
CreateHitElement("",Color3.new(255,255,255), 5, 0, 500,  0, 22)
CreateHitElement("",Color3.new(255,255,255), 5, 0, 500,  0, 22)
CreateHitElement("BARANWARE V",Color3.new(0,150,0), 5, 0, 500,  0, 22)
