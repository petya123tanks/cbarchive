...
library:CreateWatermark("effortless.cc | DEV | T.T")
library.theme.accentcolor = Color3.fromRGB(97, 97, 97)
library.theme.accentcolor2 = Color3.fromRGB(155, 155, 155)
library.theme.topheight = 45
library.theme.TextColor = Color3.fromRGB(201, 199, 196)
local library = {} 
...
local cfglocation = "effortlesscfg/" 
makefolder("effortlesscfg") 
...
makefolder("effortlesslua") 

local allluas = {} 

for _,lua in pairs(listfiles("effortlesslua")) do 
	local luaname = GSUB(lua, "effortlesslua\\", "") 
	INSERT(allluas, luaname) 
end 

RunService.RenderStepped:Wait() 

local gui = library:New("EffortLess.cc") 
...
antiaim:Element("Dropdown", "pitch", {options = {"real_down", "up", "down", "zero", "180", "random", "Normal"}}) 
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do 
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "hi" or "get effortless.cc", false, "Innocent", false, true) 
			wait(values.misc.chat["speed (ms)"].Slider/1000) 
			 
		end 
	end 
end) 
chat:Element("Dropdown", "type", {options = {"standard", "effortless"}}) 
...
RunService.RenderStepped:Connect(function(step) 
	...
	if PlayerIsAlive then 
		...
		if values.rage.angles.enabled.Toggle and not DisableAA then 
			...
			local Pitch = values.rage.angles["pitch"].Dropdown == "real_down" and -1  or values.rage.angles["pitch"].Dropdown == "up" and 1 or values.rage.angles["pitch"].Dropdown == "down" and -4 or values.rage.angles["pitch"].Dropdown == "zero" and -5 or values.rage.angles["pitch"].Dropdown == "random" and RANDOM(-10, 10)/10 or 2.5 
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current) 
	...
	if values.misc.chat["kill say"].Toggle then 
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= " for all have sinned and fall short of the glory of God" and values.misc.chat["message"].Text or "Do not be afraid for I am with you.", false, "Innocent", false, true) 
	end 
end) 
