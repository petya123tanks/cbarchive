...
ArmsColorsEnabled = false	
			...
			world:Element("Toggle", "1 tracer", nil, function(tbl)
				if tbl.Toggle then
					local nigger1 = true
						...
						mt.__namecall = newcclosure(function(self, ...)
							...
							if getnamecallmethod() == "FireServer" and self.Name == "HitPart" and tick() - lasthittick > 0.005 then
								...
								spawn(function()
									...
									for i = 1, 20 * lifetime do
										rs:Wait()
										Beam.Transparency = i / (20 * lifetime)
									end
									Beam:Destroy()
								end)
							end
			...
			RunService.RenderStepped:Connect(function(step)
				...
				if PlayerIsAlive then
								...
								if Player.Character and Player.Character:FindFirstChild("Humanoid") and not Client.DISABLED and Player.Character:FindFirstChild("Humanoid").Health > 0 and Player.Team ~= "TTT" and not Player.Character:FindFirstChildOfClass("ForceField") and GetDeg(CamCFrame, Player.Character.Head.Position) <= Stats["max fov"].Slider and Player ~= LocalPlayer then
									if Player.Team ~= LocalPlayer.Team or values.rage.aimbot.teammates.Toggle and Player:FindFirstChild("Status") and Player.Status.Team.Value ~= LocalPlayer.Status.Team.Value and Player.Status.Alive.Value then
										if Client.gun:FindFirstChild("Melee") and values.rage.aimbot["knifebot"].Toggle then
											...
											local Ray = Ray.new(Origin, (Player.Character.Head.Position - Origin).unit * 20000)
					...
					if values.rage.angles.enabled.Toggle  then
						...
						if values.rage.angles["body roll"].Dropdown == "180 treehouse" then
							Root.CFrame = Root.CFrame * CFrame.Angles(values.rage.angles["body roll"].Dropdown == "180 treehouse" and math.rad(180) or 0, 1, 0)
						...
						local Pitch = values.rage.angles["pitch"].Dropdown == "none" and CamLook.Y or values.rage.angles["pitch"].Dropdown == "up" and 1 or values.rage.angles["pitch"].Dropdown == "down" and -1 or values.rage.angles["pitch"].Dropdown == "zero" and math.huge or 10.10 or values.rage.angles["pitch"].Dropdown == "random" and math.random(-97, 68)/1 or -4 	
				...
				if method == "SetPrimaryPartCFrame" and self.Name == "Arms" then
					if values.visuals.self["third person"].Toggle and values.visuals.self["third person"].Active and LocalPlayer.Character then
						...
						if values.visuals.self["visualize silent angle"].Toggle and RageTarget then
							-- is it enabled and are we aimbotting at someone?
							wait(0.8) args[1] = CFrame.lookAt(args[1].p, RageTarget.Position) -- if yes, apply the cframe.lookat on top of the viewmodel offset
