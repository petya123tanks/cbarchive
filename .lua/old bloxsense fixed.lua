...
local function updateWatermark()
	...
	local message = cheatName .. "  |  " .. Parameters.UserType .. "  |  " .. tostring(months[month]) .. " " .. tostring(day) .. " " .. tostring(year).. "  |  Kills: " ..kills.. "  |  Deaths: " ..deaths.. "  |  KDR: " ..kdr.. "  |  Build: 5.3 " 
...
updateWatermark()
	...
	local MenuParameters = {
		...
		UserType = env.login and env.login.username or "1 tap",
	...
	do
		...
		-- Rage
		UILibrary:CreateSubSection("Rage", "Aimbot", {"Aimbot", "Settings"}, false, 0.5, 0) 
		...
		UILibrary:CreateButton({Name = "Back Tracking", Tab = "Rage", Section = "HvH", Tooltip = "Standalone. Saves previous enemy positions and allows you to hit them."})
		UILibrary:CreateSlider({Name = "Back Tracking Time", Tab = "Rage", Section = "HvH", Suffix = "ms", MinimumNumber = 0, MaximumNumber = 4000, Tooltip = "Controls how far back in time the enemy positions are saved."})
		UILibrary:CreateDropdown({Name = "Back Tracking Points", Tab = "Rage", Section = "HvH", Values = {"Head", "Chest", "Pelvis", "Arms", "Legs", "Feet"}, MultiChoice = true, "Controls which backtrack hitboxes the aimbot will consider. Note that these hitboxes must also be selected in the weapon config."})
		UILibrary:CreateButton({Name = "Extrapolation", Tab = "Rage", Section = "HvH", Tooltip = "Controls if the aimbot extrapolates where the enemy is on their own screen and allows you to predict a shot such that the enemy runs into it. Useful for preventing people from peeking you successfully."})
		UILibrary:CreateSlider({Name = "Extrapolation Tuning", Tab = "Rage", Section = "HvH", Suffix = "ms", MinimumNumber = 0, MaximumNumber = 4000, Tooltip = "Controls how much extra time ahead an enemy is extrapolated."})
		UILibrary:CreateDropdown({Name = "Extrapolation Points", Tab = "Rage", Section = "HvH", Values = {"Head", "Chest", "Pelvis", "Arms", "Legs", "Feet"}, MultiChoice = true, Tooltip = "Controls which extrapolated hitboxes the aimbot will consider. Note that these hitboxes must also be selected in the weapon config."})
		...
		do --esp and chams
			...
			function visuals.applyEsp(player)
				...
				function this.renderGeneral(info)
					...
					if section:find("Enemy") and Menu["ESP"]["Enemy ESP"]["Show Extrapolation Position"]["Toggle"]["Enabled"] and Menu["Rage"]["HvH"]["Extrapolation"]["Toggle"]["Enabled"] and player.Character and player.Character:FindFirstChild("Humanoid") and this.timePassed == 0 and info.pInfo.velocity.Magnitude > 0.01 then
						...
						local extrapolateby = info.pInfo.velocity * ((Menu["Rage"]["HvH"]["Extrapolation Tuning"]["Value"] + (game.Stats.PerformanceStats.Ping:GetValue() * 2)) / 1000) -- this is how far ahead in time we are extrapolating
...
camera:GetPropertyChangedSignal("FieldOfView"):Connect(function()
	...
	do --ANCHOR Ragebot
		...
		function ragebot.setupbacktrack(player)
			...
			this.updateBacktrack = function(upTime, deltaTime)
				...
				local backtrackTime = (Menu["Rage"]["HvH"]["Back Tracking"]["Toggle"]["Enabled"] and (Menu["Rage"]["HvH"]["Back Tracking Time"]["Value"] / 1000) or 0)
		...
		function ragebot.getpoints(enemy)
			...
			if Menu["Rage"]["HvH"]["Extrapolation"]["Toggle"]["Enabled"] and pInfo.velocity.Magnitude > 0.01 then -- since they are moving we should try to predict where they are
				local extrapolatedparam = RaycastParams.new()
				extrapolatedparam.IgnoreWater = true
				extrapolatedparam.FilterType = Enum.RaycastFilterType.Whitelist
				extrapolatedparam.FilterDescendantsInstances = {workspace.Map} -- this cannot be moved through under normal circumstances	

				local extrapolateby = pInfo.velocity * ((Menu["Rage"]["HvH"]["Extrapolation Tuning"]["Value"] + (game.Stats.PerformanceStats.Ping:GetValue() * 2)) / 1000) -- this is how far ahead in time we are extrapolating
				local extrapolatedpoints = ragebot.hitgrouptohitbox(Menu["Rage"]["HvH"]["Extrapolation Points"]["Value"]) -- what hitboxes do the hitgroups in the dropdown allow
			...
			if Menu["Rage"]["HvH"]["Back Tracking"]["Toggle"]["Enabled"] then
				local btpoints = ragebot.hitgrouptohitbox(Menu["Rage"]["HvH"]["Back Tracking Points"]["Value"])
	...
	local aimbotEnabled = Menu["Rage"]["Aimbot"]["Enabled"]["Toggle"]["Enabled"]
	local aimbotActive = Menu["Rage"]["Aimbot"]["Enabled"]["Bind"]["Active"]
	local killAll = Menu["Misc"]["Extra"]["Kill All"]["Toggle"]["Enabled"]
	local gunName = client.fgun and client.fgun.Name
	local gun = gunName and replicatedStorage.Weapons:FindFirstChild(gunName)
	local currentTarget = ragebot.currenttarget
	local now = tick()

	-- Clear targets and exit if conditions are not met
	if not (hrp and aimbotEnabled and aimbotActive) or not client.fgun or client.fgun == "none" or killAll or not gun then
		currentTarget.player = nil
		currentTarget.instance = nil
		currentTarget.position = nil
		currentTarget.modifier = nil
		currentTarget.wallbang = nil
		currentTarget.origin = nil
		currentTarget.pInfo = nil
		return
	end

	-- Check firing cooldowns
	if now - ragebot.lastshot < gun.FireRate.Value or now - ragebot.lastreload < gun.ReloadTime.Value then
		return
	end

	-- Wait for round start if enabled
	if workspace.Status.Preparation.Value and Menu["Rage"]["HvH"]["Wait For Round Start"]["Toggle"]["Enabled"] then
		return
	end

	-- Collect enemies in FOV
	local enemies = {}
	local cameraLook = camera.CFrame.LookVector
	local aimbotFOV = Menu["Rage"]["Aimbot"]["Aimbot FOV"]["Value"]

	for _, pInfo in pairs(playerInfo.storage) do
		if pInfo.alive and pInfo.enemy and not (pInfo.protected or pInfo.god) then
			local enemyPos = pInfo.character.HumanoidRootPart.Position
			local dirToEnemy = (enemyPos - camera.CFrame.p).Unit
			local angle = math.deg(math.acos(cameraLook:Dot(dirToEnemy))) * 2
			if aimbotFOV == 180 or angle < aimbotFOV then
				table.insert(enemies, {player = pInfo.player, pInfo = pInfo})
			end
		end
	end
	...
	local result
	if gun:FindFirstChild("Melee") and Menu["Rage"]["Aimbot"]["Knife Bot"]["Toggle"]["Enabled"] then
		result = ragebot.knifebot()
	else
		ragebot.currentindex = ragebot.currentindex + 1
		if ragebot.currentindex > #enemies then
			ragebot.currentindex = 1
		end
	...
	currentTarget.player = result.player
	currentTarget.instance = result.instance
	currentTarget.position = result.to[1]
	currentTarget.modifier = result.damagemodifier
	currentTarget.wallbang = result.wallbang
	currentTarget.origin = result.from[1]
	currentTarget.pInfo = result.pInfo

	-- Rescan if shot is not lethal and not melee and autoshoot disabled
	if not gun:FindFirstChild("Melee") and (result.damage < result.pInfo.humanoid.health or not Menu["Rage"]["Aimbot"]["Auto Shoot"]["Toggle"]["Enabled"]) then
		ragebot.currentindex = ragebot.currentindex - 1
	end
	...
	-- Handle double tap auto-shoot logic
	if Menu["Rage"]["Aimbot"]["Auto Shoot"]["Toggle"]["Enabled"] then
		ragebot.lastshot = now

		local dtValue = Menu["Rage"]["Aimbot"]["Double Tap"]["Value"]
		local shifting = dtValue == "Fast" and 10/64 or dtValue == "Faster" and 12/64 or dtValue == "Fastest" and 14/64

		if shifting and now - ragebot.lastdt > shifting * 2 then
			ragebot.lastdt = now
			ragebot.lastshot = now - shifting -- tickbase shifting simulation
		end
	end
end
    ...
    --debug.profileend()
		...
		localPlayer.Cash.Changed:Connect(function()
			if Menu["Misc"]["Extra"]["Infinite Money"]["Toggle"]["Enabled"] then
				task.wait(0.1)
						game:GetService("Players").LocalPlayer.PlayerGui.GUI.Cash.Text = 'Deathware'
		...
		local oldNamecall; oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
			...
			if method == "FireServer" then
				...
				elseif self.Name == hitPart.Name and not Menu["Misc"]["Extra"]["Kill All"]["Toggle"]["Enabled"] then
					...
					if Menu["Rage"]["HvH"]["Back Tracking"]["Toggle"]["Enabled"] then -- ok its a backtrack part
						local v = {
