...
Color.Size = UDim2.new(0.97, 0, 0, 2)
Color.ZIndex = 2
...
Background.Size = UDim2.new(0.97, 0, 1, 0)
...
local cfglocation = "bloxitycfg/"
makefolder("Bloxitycfg")
...
function library:New(name)
	...
	local Bloxity = Instance.new("ScreenGui")
	...
	Bloxity.Name = "Bloxity"
	Bloxity.ResetOnSpawn = false
	Bloxity.ZIndexBehavior = "Global"
	Bloxity.DisplayOrder = 420133769

	local UIScale = Instance.new("UIScale")
	UIScale.Parent = Bloxity
    ...
    game:GetService("RunService").RenderStepped:connect(function()
        cursor.Visible = Bloxity.Enabled
	...
	Menu.Size = UDim2.new(0, 600, 0, 660)
	...
	game:GetService("UserInputService").InputBegan:Connect(function(key)
		if key.KeyCode == Enum.KeyCode.RightShift then
			Bloxity.Enabled = not Bloxity.Enabled
			library.uiopen = Bloxity.Enabled
		end
	end)
...
makefolder("bloxitylua")

local allluas = {}

for _,lua in pairs(listfiles("Bloxitylua")) do
	local luaname = string.gsub(lua, "Bloxitylua\\", "")
	table.insert(allluas, luaname)
end

RunService.RenderStepped:Wait()

local gui = library:New("bloxity.tk")
...
luascripts:Element("Button", "load", {}, function()
	loadstring(readfile("bloxitylua\\"..values.luas["lua scripts"].lua.Scroll))()
end)
...
chat:Element("Toggle", "chat spam", nil, function(tbl)
	if tbl.Toggle then
		while values.misc.chat["chat spam"].Toggle do
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "standard" and "hi" or "buy Bloxity.me", false, "Innocent", false, true)
			wait(values.misc.chat["speed (ms)"].Slider/1000)
		end
	end
end)
chat:Element("Dropdown", "type", {options = {"standard", "Bloxity"}})
...
mt.__newindex = function(self, i, v)
	...
	if self:IsA("Humanoid") and i == "CameraOffset" then
        if values.rage.angles.enabled.Toggle and values.rage.angles["body roll"].Dropdown == "180 treehouse" and not DisableAA then
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)
	...
	if values.misc.chat["kill say"].Toggle then
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "looks like you don't have bloxity.tk", false, "Innocent", false, true)
	end
end)
