...
makefolder("deadware.pub")
makefolder("deadware.pub/cfgs")
makefolder("deadware.pub/luas")
...
_Text.Text = "deadware.pub | deadware_pub | 16 October 2022"
...
local Window = library:CreateWindow("deadware.pub")
...
SettingsTabConfig:CreateButton("Load config", function()
	local path = "deadware.pub/cfgs/" .. library.flags["cfg_name"] .. ".dead"
	if isfile(path) then
...
for _, v in pairs(listfiles("deadware.pub/luas")) do
...
SettingsTabLuas:CreateButton("Load", function()
	loadstring(readfile("deadware.pub/luas/" .. library.flags["lua_select"][1]))()
end)
...
RunService.RenderStepped:Connect(function(fps)
	...
	if library.flags["wm_enabled"] then
		...
		if date:find("0") or date:find("1") then
			_Text.Text = " deadware.pub | [until " .. date .. ", uid:" .. uid .. "] | " .. tostring(gtime["day"]) .. " " .. getMonthName(gtime["month"]) .. " " .. tostring(gtime["year"]) .. " "
		else
			_Text.Text = " deadware.pub | [" .. date .. ", uid:" .. uid .. "] | " .. tostring(gtime["day"]) .. " " .. getMonthName(gtime["month"]) .. " " .. tostring(gtime["year"]) .. " "
		end
...
Player.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function()      
	...
	if library.flags["kill_say"] then
		local tbl = {"Super Idol的笑容都沒你的甜八月正午的陽光都沒你耀眼熱愛 105 °C的你滴滴清純的蒸餾水你不知道你有多可愛跌倒後會傻笑著再站起來你從來都不輕言失敗對夢想的執著一直不曾更改很安心 當你對我說不怕有我在放著讓我來勇敢追自己的夢想那堅定的模樣";"1 sit nn dog";"bloxsense.hui ";"bloxsense.lol";"deadware\'d ";"invaded tap";"femboy\' d";"sorry pls";"1 by deadware.pub";"1 ez win";"r u femboy?"}
